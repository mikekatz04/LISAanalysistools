...
pip install lisaanalysistools-cuda13x
```
...
Note that the `cuda` backend is an alias for either `cuda11x`, `cuda12x`, `cuda13x`. If any is available, then the `cuda` backend is available.
