...
class PSDwithGBPriorWrap:
    ...
    def logpdf(self, coords, inds, supps=None, branch_supps=None):
        ...
        mbh_logpdf_tmp = (
            self.priors["mbh"]
            .logpdf(mbh_params.reshape(-1, mbh_params.shape[-1]))
            .reshape(mbh_params.shape[:-1])
            .sum(axis=-1)
        )
        ...
        if gb_inds.sum() > 0:  # more than zero binaries
            ...
            noise_index_all = xp.repeat(
                xp.arange(ntemps * nwalkers)[:, None], gb_inds.shape[-1], axis=-1
            )[gb_inds_in].astype(np.int32)
        ...
        all_logpdf = (gb_logpdf + psd_logpdf + galfor_logpdf + mbh_logpdf).reshape(
            coords["psd"].shape[:2]
        )
...
class UpdateNewResidualsPSD(Update):
    ...
    def __call__(self, iter, last_sample, sampler):
        ...
        sampler._priors["all_models_together"].walker_inds = np.repeat(
            np.arange(nwalkers_pe)[:, None], gb_nleaves_max, axis=-1
        )  # [new_info.gb_info["cc_inds"][gb_inds_generate]]
        sampler._priors["all_models_together"].walker_inds_map = gb_inds_generate
        sampler._priors["all_models_together"].gb_inds = new_info.gb_info["cc_inds"][
            gb_inds_generate
        ]
        ...
        new_lp_here = sampler.compute_log_prior(
            last_sample.branches_coords,
            inds=last_sample.branches_inds,
            supps=last_sample.supplimental,
        )
        logp = new_mbh_prior_vals + new_lp_here[0]
        logl = sampler.compute_log_like(
            last_sample.branches_coords,
            inds=last_sample.branches_inds,
            supps=last_sample.supplimental,
            logp=new_lp_here,
...
def run_psd_pe(gpu, comm, head_rank):
    ...
    if "gb_inds" in generated_info:
        ...
        gpu_priors = {
            "gb": GBPriorWrap(
                gf_information.gb_info["pe_info"]["ndim"],
                ProbDistContainer(gpu_priors_in, use_cupy=True),
            )
        }

        gb_params = gf_information.gb_info["cc_params"][
            gb_inds_generate
        ]  # [gf_information.gb_info["cc_inds"][gb_inds_generate]]
        walker_inds = np.repeat(
            np.arange(nwalkers_pe)[:, None], gb_nleaves_max, axis=-1
        )  # [gf_information.gb_info["cc_inds"][gb_inds_generate]]
    ...
    prior_wrap = {
        "all_models_together": PSDwithGBPriorWrap(
            gb,
    ...
    if "last_state" in psd_info:
        ...
        last_sample = State(
            {"psd": coords_psd, "galfor": coords_galfor},
            inds={"psd": inds_psd, "galfor": inds_galfor},
        )

    fd = xp.asarray(gf_information.general_info["fd"])
    data = [
        xp.asarray(generated_info["data"][0]),
        xp.asarray(generated_info["data"][1]),
    ]
    ...
    mbh_priors_in = (
        np.full(nwalkers_pe, np.inf)
        if "mbh_prior_vals" not in generated_info
        else generated_info["mbh_prior_vals"].copy()
    )
    ...
    lp = sampler_mix.compute_log_prior(
        state_mix.branches_coords,
        inds=state_mix.branches_inds,
        supps=state_mix.supplimental,
    )
    ll = sampler_mix.compute_log_like(
        state_mix.branches_coords,
        inds=state_mix.branches_inds,
        supps=state_mix.supplimental,
        logp=lp,
    ...
    out = sampler_mix.run_mcmc(
        state_mix,
        nsteps_mix,
        progress=psd_info["pe_info"]["progress"],
        thin_by=thin_by,
        store=True,
    )
