...
from lisatools.utils.utility import AET, generate_noise_fd
...
def run_mbh_search(
    ...
    split_inds = np.split(
        np.arange(num_t_ref_bins), np.arange(split_val, num_t_ref_bins, split_val)
    )[time_split]
    ...
    psd_best = psd_reader.get_chain()["psd"][
        psd_reader.get_log_like() == psd_reader.get_log_like().max()
    ].squeeze()
    galfor_best = psd_reader.get_chain()["galfor"][
        psd_reader.get_log_like() == psd_reader.get_log_like().max()
    ].squeeze()
    ...
    psd_cpu = np.asarray(
        [
            np.tile(A_psd, (all_data_cpu.shape[1], 1)),
            np.tile(E_psd, (all_data_cpu.shape[1], 1)),
            np.tile(np.full_like(A_psd, 1e10), (all_data_cpu.shape[1], 1)),
        ]
    )
    ...
    fp = (
        ...
        + f"_{time_split}_run_{num_run}.h5"
    )
    ...
    while iteration == 0 or np.any(ll_vals[0].max() - ll_vals[0] > 50.0):
        ...
        out = sampler.run_mcmc(
            {"mbh": current_start_points.reshape(ntemps, nwalkers, 1, ndim)},
            nsteps,
            progress=False,
            thin_by=50,
            burn=0,
...
class ParallelMBHSearchControl:
    ...
    def launch_search_process(self, i, current_status, current_time_segment_status):
        ...
        try:
            while (
                current_time_segment_status[time_split_ind]
                or not self.output_points_info["still_running"][time_split_ind]
            ):
                time_split_ind += 1
        ...
        if self.verbose:
            print(
                ...
                self.proc_inds[i],
                time_split_ind,
                current_time_segment_status,
            )

        self.comm.send(
            {
                "best_points": np.asarray(self.output_points_info["best_points"]),
                "time_split": time_split_ind,
                "num_run": self.output_points_info["num_run_per_split"][time_split_ind],
                "total_splits": self.time_splits,
            },
            dest=self.proc_inds[i],
        )
    ...
    def search_process_control(self):
        ...
        while run:
            ...
            self.comm.send(
                {
                    "output_points": new_output_points,
                    "mbh_best": new_best_point,
                    "time_split": time_split,
                    "finished_this_split": finished_this_split,
                },
                dest=self.head_rank,
            )
    ...
    def run_parallel_mbh_search(self, testing_time_split=None):
        ...
        if testing_time_split is not None:
            ...
            output_points, mbh_best = run_mbh_search(
                ...
                testing_time_split,
                self.time_splits,
                np.asarray(self.output_points_info["best_points"]),
                self.output_points_info["num_run_per_split"][testing_time_split],
            )
            breakpoint()
    ...
    def prune_via_matching(self):
        ...
        best_points_in = self.settings["mbh"]["transform"].both_transforms(
            best_points, return_transpose=True
        )
        ...
        A_noise = generate_noise_fd(
            self.settings["general"]["fd"],
            self.settings["general"]["df"],
            func=get_sensitivity,
            sens_fn="noisepsd_AE",
            model="sangria",
            includewd=1.0,
        )
        E_noise = generate_noise_fd(
            self.settings["general"]["fd"],
            self.settings["general"]["df"],
            func=get_sensitivity,
            sens_fn="noisepsd_AE",
        ...
        for i in range(data_channels_AET.shape[0]):
            for j in range(i, data_channels_AET.shape[0]):
                ...
                normalized_noise_weighted_corr_with_noise = (
                    -1 / 2 * (a_a + b_b - 2 * a_b)
                )  #  np.abs(a_b).real / np.sqrt(a_a * b_b).real
                if i == j:
                    out_norm_with_noise.append(
                        [
                            normalized_noise_weighted_corr_with_noise.copy(),
                            np.sqrt(b_b),
                            a_b / np.sqrt(b_b),
                        ]
                    )
        ...
        prune_keep = np.delete(
            np.arange(data_channels_AET.shape[0]), np.unique(prune_remove).astype(int)
        )

        output_points_pruned = np.asarray(
            [self.output_points_info["output_points"][i] for i in prune_keep]
        )
        best_points_pruned = np.asarray(
            [self.output_points_info["best_points"][i] for i in prune_keep]
        )
