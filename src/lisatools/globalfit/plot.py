...
def produce_sky_plot(current_info, save_file=None, fig=None):
    ...
    mbh_lam_L, mbh_sinbeta_L, mbh_psi_L, mbh_tc_L = current_info.mbh_info["cc_params"][
        ll_mbh_ind_max, :, np.array([7, 8, 9, 10])
    ]

    mbh_tc_SSB, mbh_lam_SSB, mbh_beta_SSB, mbh_psi_SSB = LISA_to_SSB(
        mbh_tc_L, mbh_lam_L, np.arcsin(mbh_sinbeta_L), mbh_psi_L
    )
...
def make_current_plot(
    ...
    if add_gbs:
        generated_info_gb = current_info.get_data_psd(
            only_max_ll=True, include_gbs=False, include_mbhs=True, **kwargs
        )
        plt.loglog(
            current_info.general_info["fd"],
            2
            * current_info.general_info["df"]
            * np.abs(generated_info_1["data"][0] - generated_info_gb["data"][0]) ** 2,
        )

    if add_mbhs:
        generated_info_mbh = current_info.get_data_psd(
            only_max_ll=True, include_gbs=True, include_mbhs=False, **kwargs
        )
        plt.loglog(
            current_info.general_info["fd"],
            2
            * current_info.general_info["df"]
            * np.abs(generated_info_1["data"][0] - generated_info_mbh["data"][0]) ** 2,
        )

    plt.loglog(
        current_info.general_info["fd"],
        2 * current_info.general_info["df"] * np.abs(generated_info_1["data"][0]) ** 2,
    )
...
class RunResultsProduction:
    ...
    def build_plots(self, current_info):
        ...
        make_current_plot(
            ...
            add_mbhs=self.add_mbhs,
            add_gbs=self.add_gbs,
            **self.kwargs,
        )

        mbh_save_file = base_save_file + f"_mbh_posterior_leaf.png"
        produce_mbh_plots(
            current_info.mbh_info["reader"],
            current_info.mbh_info["cc_params"].shape[1],
            discard=0,
            save_file=mbh_save_file,
            fig=None,
        )

        gb_save_file = base_save_file + f"_gb_posterior.png"
        produce_gbs_plots(
            current_info.gb_info["reader"], discard=0, save_file=gb_save_file, fig=None
        )

        psd_save_file = base_save_file + f"_psd_posterior.png"
        produce_psd_plots(
            current_info.psd_info["reader"],
            save_file=psd_save_file,
            discard=0,
            fig=None,
        )
