...
def build_gb_catalog(current_info, gpu, **kwargs):
    ...
    generated_info = current_info.get_data_psd(
    ...
    info_together = np.concatenate(
        [
            output_information.groups[:, None],
            output_information.confidence[:, None],
            output_information.snr[:, None],
            output_information.params,
        ],
        axis=1,
    )
    ...
    for i, group in enumerate(kept_groups):
        ...
        tmp = np.where(info_together[:, 4] == params_group[median_ind, 1])[0][
            0
        ]  # 4 should be frequency
        median_inds_out[i] = (
            tmp  # tmp is a reference ot order in info_together after removed groups for low confidence
        )
    ...
    df["coslat"] = np.cos(np.pi / 2.0 - df["Ecliptic Latitude"])
    ...
    df_meta = pd.DataFrame(
        {
            "Observation Time": [current_info.general_info["Tobs"]],
            "parent": [None],
    ...
    # exit(0)
    for i, (sub_df_name, index, store_file) in enumerate(
...
def build_mbh_catalog(current_info, gpu, **kwargs):
    ...
    samp_keys = [
        ...
        "Merger Phase",
        "cos inclination",
        "Ecliptic Longitude (L-frame)",
        "sin ecliptic latitude (L-Frame)",
        "Polarization (L-frame)",
        "Detector Merger Time",
    ]
    ...
    for leaf_i in range(mbh_leaves):
        ...
        tL, lamL, betaL, psiL = (
            ...
            df["Ecliptic Latitude (L-Frame)"],
            df["Polarization (L-frame)"],
        )
        ...
        # store samples
        df.to_hdf(
            current_info.general_info["file_information"]["mbh_main_chain_file"],
            name_index[leaf_i] + "_chain",
            mode="a",
            complevel=9,
        )
