...
class UpdateNewResidualsMBH(Update):
    ...
    def __call__(self, iter, last_sample, sampler):
        ...
        if self.verbose:
            print(
                f"Finished subbing in new data. Acceptance fraction: {accept.sum() / accept.shape[0]}"
            )
...
def run_mbh_pe(gpu, comm, head_rank):
    ...
    data_fin = xp.asarray(
        [
            generated_info["data"][0],
            generated_info["data"][1],
            np.zeros_like(generated_info["data"][1]),
        ]
    )
    psds_fin = xp.asarray(
        [
            generated_info["psd"][0],
            generated_info["psd"][1],
            np.full_like(generated_info["psd"][1], 1e10),
        ]
    )
    ...
    if hasattr(start_state, "betas_all") and start_state.betas_all is not None:
        ...
        betas_all = np.tile(
            make_ladder(mbh_info["pe_info"]["ndim"], ntemps=ntemps_pe),
            (mbh_info["pe_info"]["nleaves_max"], 1),
        )
    ...
    out = sampler_mix.run_mcmc(
        start_state,
        nsteps_mix,
        progress=mbh_info["pe_info"]["progress"],
        thin_by=thin_by,
        store=True,
    )
