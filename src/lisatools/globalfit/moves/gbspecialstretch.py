...
class _ProposeTimer:
    ...
    def report(self, total: float, top=None) -> str:
        ...
        top = top or (
            ...
            "fstat_grid_fit",
            # Joint two-band cold-chain substitution: like fstat_grid_fit,
            # real propose time spent OUTSIDE run_proposal, so it belongs in
            # the tracked total rather than reading as an accounting hole.
            "pair_borrow",
        )
...
class GBSpecialBase(GlobalFitMove, GroupStretchMove, Move, LISAToolsParallelModule):
    ...
    # ========== PAIR-BORROW: joint two-band cold-chain substitution ==========
    # USER DESIGN 2026-08-29.
    #
    # A source astride a sub-band edge splits into two leaves in ADJACENT
    # bands (the 20.380377 mHz flagship: 30 leaves in band 1141, 22 in
    # 1142, quad SNR 302.8 vs 76.3, corr -0.747, total power conserved).
    # Adjacent bands land in DIFFERENT residue classes -- 1141 % 9 == 7,
    # 1142 % 9 == 8 -- so they are NEVER open in the same scheduling unit
    # (``band_inds % units == remainder``, gbbands.py:4930). The update
    # that would drop one leaf while boosting the other is therefore not
    # expressible at all: a BLOCK-GIBBS TRAP, not a mixing failure. No
    # scan reordering escapes it (the conditionals are unchanged by order)
    # and no single-band exchange escapes it either -- handing a poisoned
    # walker the clean 1141 leaf while it still holds 1142 double-counts,
    # so both sides of such a swap score worse and it is doubly rejected
    # (scoping report 2026-08-29).
    #
    # THE MOVE. Take a DONOR walker that already holds the clean
    # single-leaf configuration over the pair and substitute its
    # cold-chain pair content into a flagged recipient. How the candidate
    # is GENERATED is deliberately unconstrained -- a donor copy today, a
    # "pretend and rework" block later. What makes it safe is that
    # ACCEPTANCE IS PRICED ON THE TRUE LIKELIHOOD in the recipient's OWN
    # residual context, so the cold chain can never end up worse than it
    # started; and the gate scores BOTH bands jointly, which is precisely
    # what makes it a two-band move despite every stage that produced the
    # content having run per-band.
    #
    # WHY THE GATE IS A WHOLE-WALKER lnL AND NOT A TWO-BAND ONE. A band is
    # 135 bins but a GB template's likelihood window is 3,240-5,400 bins
    # (Q8, the scheduling-vs-containment ruling): a template seeded in
    # band b has support across ~24-40 bands. Scoring the substitution on
    # the two bands' 270-bin extents would capture only a few percent of
    # the change and would accept/reject on noise. The parent
    # ``likelihood()`` is per-walker and unambiguous, and it is exactly
    # the quantity :meth:`check_ll_inject` reconciles against, so the
    # user's own named analogue is followed rather than re-invented.
    #
    # SEARCH ONLY. Donor choice is state-dependent (best cold logL), which
    # carries no detailed-balance cost only because nothing is priced on a
    # pretended density -- the gate is on true improvement.
    # :meth:`_pair_borrow_enabled` HARD-refuses on the ``replace_pe_stage``
    # install stamp and outside ``GB_MODE=search``, so no env setting can
    # arm it in PE (stronger than the ``auto`` convention, which an
    # explicit ``=1`` can override).
    #
    # TWO KNOBS, BOTH OFF IN CODE:
    #   ``GB_PAIR_BORROW=1``       -- MEASURE: enumerate, stage the
    #       substitution into the residual, record the TRUE dll, then
    #       REVERT. Touches no chain state, so it answers "would this fix
    #       the flagship?" with zero risk before anything destructive runs.
    #   ``GB_PAIR_BORROW_APPLY=1`` -- also KEEP accepted substitutions.
    #       Requires the measure knob. UNVERIFIED end-to-end on this
    #       laptop (no GPU, and the integration flow test is barred), so
    #       it must pass a cluster smoke gate before a production arm.

    def _pair_borrow_enabled(self) -> bool:
        """Is the pair-borrow MEASUREMENT pass on for this move?

        Default OFF in code. The PE refusals below are unconditional: they
        are checked AFTER the knob so an explicit ``=1`` cannot buy its way
        past them, which is what "provably inert in PE" requires.
        """
        mode = os.environ.get("GB_PAIR_BORROW", "0").strip().lower()
        if mode in ("", "0", "off", "false"):
            return False
        # HARD PE INTERLOCK -- the install-site stamp wins over the env.
        if bool(getattr(self, "replace_pe_stage", False)):
            return False
        if os.environ.get("GB_MODE", "").strip().lower() != "search":
            return False
        return True

    def _pair_borrow_apply_enabled(self) -> bool:
        """Whether an ACCEPTED substitution is kept (vs measured and reverted).

        Strictly narrower than :meth:`_pair_borrow_enabled`: the measure
        pass gates it, so PE inertness is inherited and cannot be bypassed.
        """
        if not self._pair_borrow_enabled():
            return False
        mode = os.environ.get("GB_PAIR_BORROW_APPLY", "0").strip().lower()
        return mode not in ("", "0", "off", "false")

    @staticmethod
    def _pair_borrow_candidates(occ, cold_ll, side=1, max_pairs=8):
        """Flagged ``(recipient_walker, low_band, donor_walker)`` triples.

        ``occ`` is the ``(nwalkers, num_bands)`` count of ALIVE COLD leaves.
        Over each adjacent pair ``(low, low + 1)``:

        * a RECIPIENT holds >= 2 leaves across the pair -- the split
          signature the flagship shows (one leaf each side of the edge, or
          two on one side);
        * a DONOR holds exactly 1 -- the clean single-leaf configuration
          that 7 of 24 walkers already reach at the same total SNR.

        A pair with no donor is skipped: there is nothing to borrow. The
        donor is the surviving candidate with the best cold logL.

        Pairs are NORMALISED to their low band, so the left and right scans
        name the same pair; ``side`` therefore does not change WHICH pairs
        exist, only the order they are offered in (ascending low band for
        ``+1``, descending for ``-1``). That is what "one side at a time"
        buys once ``max_pairs`` truncates -- the two sides spend the budget
        on opposite ends of the band grid.
        """
        occ = np.asarray(occ)
        cold_ll = np.asarray(cold_ll, dtype=float)
        if occ.ndim != 2 or occ.shape[1] < 2:
            return []
        nwalkers, num_bands = occ.shape
        side = 1 if int(side) >= 0 else -1

        rows = []
        for low in range(num_bands - 1):
            tot = occ[:, low] + occ[:, low + 1]
            donors = np.where(tot == 1)[0]
            if donors.size == 0:
                continue
            recips = np.where(tot >= 2)[0]
            if recips.size == 0:
                continue
            donor = int(donors[int(np.argmax(cold_ll[donors]))])
            for w in recips:
                rows.append((int(tot[w]), int(low), int(w), donor))

        # Most-split pairs first; then the side-dependent band sweep; then
        # walker, so the order is total and reproducible.
        rows.sort(key=lambda r: (-r[0], r[1] * side, r[2]))
        return [(w, low, d) for _tot, low, w, d in rows[: int(max_pairs)]]

    @staticmethod
    def _pair_borrow_accept(dll, eps=0.0) -> bool:
        """STRICT improvement on the true likelihood, or no substitution.

        This is the whole safety argument: the proposal may be generated by
        any pretended context we like, but nothing is kept unless the
        recipient's OWN cold-chain likelihood genuinely improved. A
        non-finite delta (a blown-up template) is a refusal, never an
        accept.
        """
        try:
            d = float(dll)
    ...
    @staticmethod
    def _pair_borrow_log_line(name, propose, side, attempted, subs) -> str:
        """The one greppable ``[GB_PAIRBORROW]`` line per propose.

        ``subs`` is ``[(recipient, donor, low_band, true_dll), ...]``.
        """
        parts = ",".join(
            f"w{int(w)}<-w{int(d)}:({int(b)},{int(b) + 1}):{float(x):+.3e}"
            for w, d, b, x in subs
        )
        line = (
            f"[GB_PAIRBORROW {name}] propose={int(propose)} "
            f"side={int(side):+d} attempted={int(attempted)} "
            f"substituted={len(subs)}"
        )
        return line + (f" subs={parts}" if subs else "")

    def _pair_borrow_cold_occupancy(self, band_sorter):
        """``(nwalkers, num_bands)`` count of ALIVE COLD leaves."""
        cold = (band_sorter.temp_inds == 0) & band_sorter.inds
        w = _to_numpy(band_sorter.walker_inds[cold]).astype(int)
        b = _to_numpy(band_sorter.band_inds[cold]).astype(int)
        occ = np.zeros((self.nwalkers, self.num_bands), dtype=int)
        if w.size:
            np.add.at(occ, (w, b), 1)
        return occ

    def _pair_borrow_fill(self, model, params_in, walker_row, N_vals, factor):
        """Adjust the residual of ONE walker row by ANOTHER walker's templates.

        This is the only genuinely new primitive the move needs, and it is
        the whole "borrow": identical to
        :meth:`adjust_sources_in_residual_buffer` except the walker vector
        handed to the engine is FORCED to ``walker_row`` instead of being
        read from the sources' own labels. Because every buffer fill index
        map addresses the parent ACA by WALKER and never by temperature
        (see :meth:`_vertical_pairs`), editing the recipient's parent row
        is what makes every rung of that walker -- t = 0 included -- see the
        borrowed context.

        ``factor=+1`` restores templates INTO the residual, ``-1``
        subtracts them (the convention of
        :meth:`adjust_sources_in_residual_buffer`).
        """
        if params_in is None or int(params_in.shape[0]) == 0:
            return
        walkers = self.xp.full(
            int(params_in.shape[0]), int(walker_row), dtype=self.xp.int32
        )
        self._likelihood_engine.fill_template(
            model.analysis_container_arr, params_in, walkers, N_vals,
            factor=factor, waveform_kwargs=self.waveform_kwargs,
        )

    def _pair_borrow_rows(self, band_sorter, walker, low_band):
        """Alive COLD row indices of ``walker`` over the pair ``(low, low+1)``."""
        sel = (
            (band_sorter.temp_inds == 0)
            & (band_sorter.walker_inds == int(walker))
            & band_sorter.inds
            & (
                (band_sorter.band_inds == int(low_band))
                | (band_sorter.band_inds == int(low_band) + 1)
            )
        )
        return self.xp.where(sel)[0]

    def _run_pair_borrow(self, model, band_sorter):
        """Measure -- and optionally keep -- joint two-band substitutions.

        Runs on the parent residual with every cold source subtracted (i.e.
        AFTER ``run_proposal`` has closed every unit), so it is outside the
        per-unit orthogonality bookkeeping entirely: ``GB_ORTHO_LL_CHECK``
        reconciles per unit and never observes this pass. Each candidate is
        staged, measured against the TRUE per-walker likelihood, and either
        kept or exactly reverted, so the residual this method returns is
        consistent with the chain state by construction and the
        propose-level 1e-4 reconcile has nothing to repair.
        """
        if not self._pair_borrow_enabled():
            return
        propose = GBSpecialBase._branch_propose_counts.get(self.branch_name, 0)
        # ONE SIDE AT A TIME (user design item 6): alternate per propose.
        side = 1 if (propose % 2 == 0) else -1
        max_pairs = int(os.environ.get("GB_PAIR_BORROW_MAX_PAIRS", "8"))
        eps = float(os.environ.get("GB_PAIR_BORROW_EPS", "0.0"))
        do_apply = self._pair_borrow_apply_enabled()

        occ = self._pair_borrow_cold_occupancy(band_sorter)
        ll0 = _to_numpy(model.analysis_container_arr.likelihood()).real.astype(float)
        cands = self._pair_borrow_candidates(
            occ, ll0, side=side, max_pairs=max_pairs
        )

        subs = []
        for (w, low, donor) in cands:
            own = self._pair_borrow_rows(band_sorter, w, low)
            don = self._pair_borrow_rows(band_sorter, donor, low)
            if int(don.shape[0]) == 0:
                continue
            own_p = band_sorter.coords_in[own]
            own_N = band_sorter.N_vals[own]
            don_p = band_sorter.coords_in[don]
            don_N = band_sorter.N_vals[don]

            before = float(
                _to_numpy(model.analysis_container_arr.likelihood()).real[w]
            )
            # Stage: lift the recipient's own pair content out of ITS row,
            # push the donor's in.
            self._pair_borrow_fill(model, own_p, w, own_N, +1)
            self._pair_borrow_fill(model, don_p, w, don_N, -1)
            after = float(
                _to_numpy(model.analysis_container_arr.likelihood()).real[w]
            )
            dll = after - before

            if do_apply and self._pair_borrow_accept(dll, eps=eps):
                self._pair_borrow_apply(band_sorter, w, donor, low, own, don)
                subs.append((w, donor, low, dll))
            else:
                # Exact revert -- inverse ops in the inverse order.
                self._pair_borrow_fill(model, don_p, w, don_N, +1)
                self._pair_borrow_fill(model, own_p, w, own_N, -1)
                if not do_apply and self._pair_borrow_accept(dll, eps=eps):
                    # Measurement mode still REPORTS what it would have
                    # taken; that report is the point of the pass.
                    subs.append((w, donor, low, dll))

        logger.info(
            self._pair_borrow_log_line(
                self.name, propose, side, len(cands), subs
            )
        )

    def _pair_borrow_apply(self, band_sorter, w, donor, low, own_rows, don_rows):
        """Write an accepted substitution into the COLD chain state.

        Kills the recipient's pair leaves and re-seeds an equal-or-larger
        set of its DEAD cold rows with the donor's coordinates, relabelled
        onto the recipient. Leaf counts over the pair can only shrink or
        match the donor's, so no cap can be breached by this write.

        UNVERIFIED END-TO-END on a laptop (no GPU; the integration flow
        test is barred by policy). Gated behind ``GB_PAIR_BORROW_APPLY``
        and MUST pass a cluster smoke gate before any production arm.
        """
        xp = self.xp
        main = getattr(band_sorter, "main_band_sorter", None) or band_sorter
        m_own = band_sorter.inds_main_band_sorter[own_rows]
        m_don = band_sorter.inds_main_band_sorter[don_rows]

        # Free the recipient's current pair content.
        main.inds[m_own] = False

        # Re-seed from the recipient's own DEAD cold rows, preferring the
        # slots just vacated so the write stays inside the pair's cells.
        need = int(m_don.shape[0])
        dead = m_own[:need]
        if int(dead.shape[0]) < need:
            spare = xp.where(
                (main.temp_inds == 0)
                & (main.walker_inds == int(w))
                & (~main.inds)
            )[0][: need - int(dead.shape[0])]
            dead = xp.concatenate([dead, spare])
        take = int(dead.shape[0])
        if take == 0:
            return

        main.coords[dead] = main.coords[m_don[:take]]
        main.band_inds[dead] = main.band_inds[m_don[:take]]
        main.inds[dead] = True
        main.special_band_inds[dead] = main.get_special_band_index(
            main.temp_inds[dead], main.walker_inds[dead], main.band_inds[dead]
        )
    ...
    def propose(self, model, state):
        ...
        # PAIR-BORROW (default OFF; GB_PAIR_BORROW). Placed HERE, after
        # run_proposal has closed every band unit, so the pass sees the
        # parent residual in its fully-subtracted state and sits outside
        # the per-unit orthogonality bookkeeping entirely -- it stages,
        # measures against the TRUE per-walker likelihood, and either keeps
        # or exactly reverts, leaving the residual consistent either way.
        with tm.span("pair_borrow"):
            self._run_pair_borrow(model, band_sorter)
