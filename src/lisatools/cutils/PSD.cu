...
// CPU/GPU class-name aliases for XYZSensitivityMatrix / NoiseLevels live in
// PSD.hpp (sprint rule: the alias block sits at the top of the header that
 ...
 */
static CUDA_DEVICE double omega(double f) { return 2.0 * M_PI * f; }
 ...
 */
static CUDA_DEVICE double d_times_omega(double d, double f) {
  return d * omega(f);
}
 ...
 */
static CUDA_DEVICE double s_wl(double d, double f) {
 ...
 */
static CUDA_DEVICE double c_wl(double d, double f) {
 ...
 */
static CUDA_DEVICE double c_2wl(double d, double f) {
 ...
 */
static CUDA_DEVICE int link_to_index(int link) {
  switch (link) {
    case 12:
      return 0;
    case 23:
    ...
    default:
      ...
      throw std::invalid_argument(
          "Bad link ind. Must be 12, 23, 31, 13, 32, 21.");
#endif
      return -1;
  }
}
 ...
 *
 * Computes the test mass (acceleration) noise and optical metrology system
 * noise power spectral densities at a given frequency.
 *
 ...
 * @param[in] return_relative_frequency If true, return in relative frequency
 * units.
 */
CUDA_DEVICE
void NoiseLevels::get_testmass_noise(double* S_tm, double f, double Sa_a_in,
  ...
  double Sa_a = Sa_a_in * Sa_a_in * (1.0 + pow(f_knee_tm / f, 2)) *
                (1.0 + pow(f / f_break_tm, 4));
  double Sa_d = Sa_a * pow(2.0 * M_PI * f, -4.0);  // In displacement
  double Sa_nu =
      Sa_d * pow(2.0 * M_PI * f / Clight, 2);  // In relative frequency
  double S_tm_analytic;
 ...
 */
static CUDA_DEVICE void invert_3x3_hermitian(double c00, cmplx c01, cmplx c02,
                                             double c11, cmplx c12, double c22,
                                             double& i00, cmplx& i01,
                                             cmplx& i02, double& i11,
                                             cmplx& i12, double& i22,
                                             double& det) {
  ...
  // Inverse = adjugate / det (adjugate = conjugate transpose of cofactor
  // matrix)
  i00 = C00 * inv_det;
  i11 = C11 * inv_det;
  i22 = C22 * inv_det;
 ...
 * Structure: [[i00, i01, i02], [conj(i01), i11, i12], [conj(i02), conj(i12),
 * i22]]
 *
 * @param d_X, d_Y, d_Z Complex data vector d.
 * @param i00, i01, i02, i11, i12, i22 Upper triangle of inverse covariance
 * matrix.
 * @return Real-valued quadratic form (imaginary part cancels for Hermitian).
 */
CUDA_DEVICE
double quadratic_form(cmplx d_X, cmplx d_Y, cmplx d_Z, double i00, cmplx i01,
                      cmplx i02, double i11, cmplx i12, double i22) {
  ...
  double quad = (gcmplx::conj(d_X) * inv_d_X + gcmplx::conj(d_Y) * inv_d_Y +
                 gcmplx::conj(d_Z) * inv_d_Z)
                    .real();
 ...
 *   Flattened: data_in[(data_index * 3 + channel) * (num_times * num_freqs) +
 * time_idx * num_freqs + freq_idx]
 *
 * Loop order (time-major, same as data for coalesced access):
 *   idx = t_idx * num_freqs + f_idx
 *   Adjacent threads process adjacent (t,f) pairs, ensuring coalesced data
 * access.
 *
 * @param like_contrib Output partial likelihood contributions per block/PSD.
 * @param f_arr Frequency array of length num_freqs.
 * @param data_in Flattened complex TDI data: (num_datasets, 3, num_times *
 * num_freqs).
 * @param data_index_all Maps each PSD to a dataset index, length num_psds.
 * @param time_index_all Array of time indices for orbit config lookup, length
 * num_times.
  ...
  double compensation[1];
    ...
    for (unsigned int s = blockDim.x / 2; s > 0; s >>= 1) {
      if (tid < s) {
        ...
        double y =
            like_vals[tid + s] - compensation[tid + s] - compensation[tid];
        double t = like_vals[tid] + y;
        compensation[tid] = (t - like_vals[tid]) - y;
        like_vals[tid] = t;
    ...
    if (tid == 0) {
#ifdef __CUDACC__
      like_contrib[psd_i * gridDim.x + blockIdx.x] =
          like_vals[0] - compensation[0];
...
CUDA_KERNEL void like_sum_from_contrib(double* like_contrib_final,
                                       double* like_contrib,
                                       int num_blocks_per_psd, int num_psds) {
  int tid;
  int psd_i;
  int incr_psd;
...
void XYZSensitivityMatrix::psd_likelihood_wrap(
  ...
  // gpuErrchk(cudaMemcpy(dev_ptr, this, sizeof(XYZSensitivityMatrix),
  ...
  like_sum_from_contrib<<<grid_reduc, NUM_THREADS_LIKE>>>(
...
CUDA_DEVICE
double XYZSensitivityMatrix::oms_xx_unequal_armlength(double f, double avg_d_ij,
                                                      double avg_d_ik) {
  double _oms = 8.0 * (pow(s_wl(avg_d_ij, f), 2) + pow(s_wl(avg_d_ik, f), 2));
...
CUDA_DEVICE
cmplx XYZSensitivityMatrix::oms_xy_unequal_armlength(double f, double avg_d_ij,
                                                     double avg_d_ik,
                                                     double avg_d_jk,
                                                     double delta_d_ij) {
  cmplx _oms =
      -8.0 *
      (c_wl(avg_d_ij, f) * s_wl(avg_d_ik, f) * s_wl(avg_d_jk, f) *
       gcmplx::exp(cmplx(0.0, -1.0) *
                   d_times_omega((avg_d_ik - avg_d_jk + 0.5 * delta_d_ij), f)));

  if (generation == 2) {
    _oms = _oms *
           (4.0 * s_wl(avg_d_ij + avg_d_ik, f) * s_wl(avg_d_ij + avg_d_jk, f) *
            gcmplx::exp(cmplx(0.0, -1.0) *
                        d_times_omega((avg_d_ik - avg_d_jk), f)));
  }
...
CUDA_DEVICE
double XYZSensitivityMatrix::tm_xx_unequal_armlength(double f, double avg_d_ij,
                                                     double avg_d_ik) {
  double _tm = 8.0 * (pow(s_wl(avg_d_ij, f), 2) * (3 + c_2wl(avg_d_ik, f)) +
...
CUDA_DEVICE
cmplx XYZSensitivityMatrix::tm_xy_unequal_armlength(double f, double avg_d_ij,
                                                    double avg_d_ik,
                                                    double avg_d_jk,
                                                    double delta_d_ij) {
  cmplx _tm =
      ...
      (c_wl(avg_d_ij, f) * s_wl(avg_d_ik, f) * s_wl(avg_d_jk, f) *
       gcmplx::exp(cmplx(0.0, -1.0) *
                   d_times_omega((avg_d_ik - avg_d_jk + 0.5 * delta_d_ij), f)));
  if (generation == 2) {
    _tm *= (4.0 * s_wl(avg_d_ij + avg_d_ik, f) * s_wl(avg_d_ij + avg_d_jk, f) *
            gcmplx::exp(cmplx(0.0, -1.0) *
                        d_times_omega((avg_d_ik - avg_d_jk), f)));
  }
...
void XYZSensitivityMatrix::get_noise_tfs(
    ...
    double* tm_yy, cmplx* tm_yz, double* tm_zz, int time_index) {
  ...
  *oms_xy = oms_xy_unequal_armlength(f, avg_d[index1], avg_d[index2],
                                     avg_d[index3], delta_d[index1]);
  *tm_xy = tm_xy_unequal_armlength(f, avg_d[index1], avg_d[index2],
                                   avg_d[index3], delta_d[index1]);
  ...
  *oms_yz = oms_xy_unequal_armlength(f, avg_d[index1], avg_d[index2],
                                     avg_d[index3], delta_d[index1]);
  *tm_yz = tm_xy_unequal_armlength(f, avg_d[index1], avg_d[index2],
                                   avg_d[index3], delta_d[index1]);
...
// frequencies and time indices
CUDA_KERNEL
void get_noise_tfs_kernel(double* frequencies, int* time_indices,
                          double* oms_xx_arr, cmplx* oms_xy_arr,
                          cmplx* oms_xz_arr, double* oms_yy_arr,
                          cmplx* oms_yz_arr, double* oms_zz_arr,
                          double* tm_xx_arr, cmplx* tm_xy_arr, cmplx* tm_xz_arr,
                          double* tm_yy_arr, cmplx* tm_yz_arr,
                          double* tm_zz_arr, int num_freqs, int num_times,
                          XYZSensitivityMatrix* sensitivity_matrix) {
  ...
  for (int t_idx = start_time; t_idx < end_time; t_idx += increment_time) {
    ...
    for (int i = start; i < end; i += increment) {
      ...
      sensitivity_matrix->get_noise_tfs(
          f, &oms_xx, &oms_xy, &oms_xz, &oms_yy, &oms_yz, &oms_zz, &tm_xx,
...
void XYZSensitivityMatrix::get_noise_tfs_arr(
    double* freqs, double* oms_xx_arr, cmplx* oms_xy_arr, cmplx* oms_xz_arr,
    double* oms_yy_arr, cmplx* oms_yz_arr, double* oms_zz_arr,
    double* tm_xx_arr, cmplx* tm_xy_arr, cmplx* tm_xz_arr, double* tm_yy_arr,
    cmplx* tm_yz_arr, double* tm_zz_arr, int num_freqs, int num_times,
    int* time_indices) {
#ifdef __CUDACC__
  ...
  get_noise_tfs_kernel<<<num_blocks, NUM_THREADS>>>(
      freqs, time_indices, oms_xx_arr, oms_xy_arr, oms_xz_arr, oms_yy_arr,
      oms_yz_arr, oms_zz_arr, tm_xx_arr, tm_xy_arr, tm_xz_arr, tm_yy_arr,
      tm_yz_arr, tm_zz_arr, num_freqs, num_times, sensitivity_matrix_gpu);
  cudaDeviceSynchronize();
  gpuErrchk(cudaGetLastError());
  gpuErrchk(cudaFree(sensitivity_matrix_gpu));
#else   // __CUDACC__
  get_noise_tfs_kernel(freqs, time_indices, oms_xx_arr, oms_xy_arr, oms_xz_arr,
                       oms_yy_arr, oms_yz_arr, oms_zz_arr, tm_xx_arr, tm_xy_arr,
                       tm_xz_arr, tm_yy_arr, tm_yz_arr, tm_zz_arr, num_freqs,
                       num_times, this);
#endif  // __CUDACC__
}
...
CUDA_KERNEL
void get_inverse_det_kernel(double* c00_arr, cmplx* c01_arr, cmplx* c02_arr,
                            double* c11_arr, cmplx* c12_arr, double* c22_arr,
  ...
  for (int i = start; i < end; i += increment) {
    invert_3x3_hermitian(c00_arr[i], c01_arr[i], c02_arr[i], c11_arr[i],
                         c12_arr[i], c22_arr[i], i00_arr[i], i01_arr[i],
                         i02_arr[i], i11_arr[i], i12_arr[i], i22_arr[i],
                         det_arr[i]);
  }
}

void XYZSensitivityMatrix::get_inverse_det_arr(double* c00_arr, cmplx* c01_arr,
                                               cmplx* c02_arr, double* c11_arr,
                                               cmplx* c12_arr, double* c22_arr,
                                               double* i00_arr, cmplx* i01_arr,
                                               cmplx* i02_arr, double* i11_arr,
                                               cmplx* i12_arr, double* i22_arr,
                                               double* det_arr, int num) {
#ifdef __CUDACC__
  int num_blocks = (num + NUM_THREADS - 1) / NUM_THREADS;

  get_inverse_det_kernel<<<num_blocks, NUM_THREADS>>>(
      c00_arr, c01_arr, c02_arr, c11_arr, c12_arr, c22_arr, i00_arr, i01_arr,
      i02_arr, i11_arr, i12_arr, i22_arr, det_arr, num);

  cudaDeviceSynchronize();
  gpuErrchk(cudaGetLastError());
#else
  get_inverse_det_kernel(c00_arr, c01_arr, c02_arr, c11_arr, c12_arr, c22_arr,
                         i00_arr, i01_arr, i02_arr, i11_arr, i12_arr, i22_arr,
                         det_arr, num);
#endif
}
...
void compute_logpdf(double* logpdf_out, int* component_index, double* points,
                    double* weights, double* mins, double* maxs, double* means,
                    double* invcovs, double* dets, double* log_Js,
  ...
  for (int i = start; i < num_points; i += incr) {
    ...
    while (start_index_here < end_index_here) {
      ...
      if (j < end_index_here) {
        ...
        for (int k = 0; k < PDF_NDIM; k += 1) {
          ...
          for (int l = 0; l < PDF_NDIM; l += 1) {
            invcov_here[k][l] =
                invcovs[(k * PDF_NDIM + l) * num_components + component_here];
          }
        }
        ...
        for (int k = 0; k < PDF_NDIM; k += 1) {
          point_mapped[k] =
              ((point_here[k] - mins_here[k]) / (maxs_here[k] - mins_here[k])) *
                  2. -
              1.;
        ...
        log_norm_factor = -(double(PDF_NDIM) / 2.) * log(2 * M_PI) -
                          (1. / 2.) * log(det_here);
        ...
        // if ((blockIdx.x == 0) && (tid == 0)) printf("%d, %.10e %.10e %.10e
        // %.10e\n", component_here, log(weight_here), log(det_here),
        // kernel_sum, -(double(PDF_NDIM) / 2.) * log(2 * M_PI));
      }
#ifdef __CUDACC__
      CUDA_SYNC_THREADS;
      for (unsigned int s = 1; s < blockDim.x; s *= 2) {
        if (tid % (2 * s) == 0) {
          max_log_sum_arr[tid] =
              max(max_log_sum_arr[tid], max_log_sum_arr[tid + s]);
        }
        CUDA_SYNC_THREADS;
      }
      CUDA_SYNC_THREADS;
      ...
      if (tid == 0) {
        max_log_all = max_log_sum_arr[tid];
      }
#ifdef __CUDACC__
      CUDA_SYNC_THREADS;
...
void compute_logpdf_wrap(double* logpdf_out, int* component_index,
  ...
  if (ndim != PDF_NDIM) {
    throw std::invalid_argument("ndim in does not equal NDIM_PDF in GPU code.");
  }

#ifdef __CUDACC__
  compute_logpdf<<<num_points, PDF_NUM_THREADS>>>(
      logpdf_out, component_index, points, weights, mins, maxs, means, invcovs,
      dets, log_Js, num_points, start_index, num_components);
  cudaDeviceSynchronize();
  gpuErrchk(cudaGetLastError());
#else
  compute_logpdf(logpdf_out, component_index, points, weights, mins, maxs,
                 means, invcovs, dets, log_Js, num_points, start_index,
                 num_components);
#endif
}
...
CUDA_CALLABLE_MEMBER void lisanoises(double* Spm, double* Sop, double f,
                                     double Soms_d_in, double Sa_a_in,
  ...
  // In acceleration
  double Sa_a =
      Sa_a_in * (1.0 + pow((0.4e-3 / frq), 2)) * (1.0 + pow((frq / 8e-3), 4));
  // In displacement
  ...
  *Sop = Soms_nu;
  ...
  //     \n", frq, Sa_a_in, Soms_d_in, Sa_a, Sa_d, Sa_nu, *Spm, Soms_d, Soms_nu,
  //     *Sop);
}
                         ...
                         double *Amp_all, double *alpha_all, double *f_1_all, double *kn_all, double *f_2_all, double df, int data_length, int num_data, int num_psds)
{
  ...
  like_sum_from_contrib<<<grid_gather, NUM_THREADS_LIKE>>>(
      like_contrib_final, like_contrib, num_blocks, num_psds);
  cudaDeviceSynchronize();
  gpuErrchk(cudaGetLastError());
