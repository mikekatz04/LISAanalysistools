...
if HAS_NUMBA:
    ...
    def _sgal_device(fr, amp, alpha, sl1, kn, sl2):
        ...
        return (
            amp
            * math.exp(-(fr**alpha) * sl1)
            * fr ** (-7.0 / 3.0)
            * 0.5
            * (1.0 + math.tanh(-(fr - kn) * sl2))
        )
    ...
    def _quadratic_form_3x3(
        d_x_re, d_x_im, d_y_re, d_y_im, d_z_re, d_z_im, c00, c01, c02, c11, c12, c22
    ):
...
def psd_likelihood_xyz_cupy(
    ...
    cov = build_xyz_covariance(
        f_arr, Soms_d_in_all, Sa_a_in_all, Amp_all, alpha_all, sl1_all, kn_all, sl2_all
    )  # (num_psds, Nfreq, 3, 3)
