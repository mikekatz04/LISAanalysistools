/**
 * @file domains.hpp
 * @brief Declarations for the STFT/FD domain classes and CUDA likelihood kernels.
 *
 * This header defines:
 *  - STFTSettings / FDSettings  – lightweight POD structs holding grid parameters
 *    (number of time bins, frequency bins, channels, physical extents).
 ...
 *      compute_likelihood_contributions_kernel  – first pass: partial (d|h) and (h|h)
 *      like_sum_from_contrib_cmplx              – second pass: block-level reduction
 *
 * Memory layout conventions (row-major, zero-based indices)
 * ----------------------------------------------------------
 * data / invC (diagonal / AET mode):
 *   index = ((data_index * num_channels + channel) * num_times + t_idx) * num_freqs + f_idx
 *
...
// TDI_XYZ: uses the full 3x3 cross-channel inverse covariance (off-diagonal terms are non-zero)
// TDI_AET: uses the diagonal inverse covariance (A, E, T channels are treated as independent)
#define TDI_XYZ 0
#define TDI_AET 1
...
public:
    ...
    int num_channels; ///< Number of TDI channels (typically 2 for AET, 3 for XYZ)
...
public:
    CUDA_CALLABLE_MEMBER
    FDSettings(int num_freqs_, int num_channels_,
               double f_min_, double f_max_, double df_)
        : STFTSettings(1, num_freqs_, num_channels_, 0.0, f_min_, f_max_, 0.0, df_) {}
};
...
public:
    cmplx *data;    ///< Complex data array, shape [num_data, num_channels, num_times, num_freqs]
    cmplx *invC;    ///< Inverse-covariance array; shape depends on TDI mode:
                    ///<   diagonal (AET): [num_noise, num_channels, num_times, num_freqs]
                    ///<   full matrix (XYZ): [num_noise, num_channels, num_channels, num_times, num_freqs]
    int num_data;   ///< Number of independent data realisations (e.g. MCMC walkers)
    int num_noise;  ///< Number of independent noise/PSD realisations
    int tdi_type;   ///< TDI channel configuration type (TDI_XYZ or TDI_AET)

    CUDA_CALLABLE_MEMBER
    STFTDomain(int num_times_, int num_freqs_, int num_channels_,
               double t0_, double f_min_, double f_max_,
               double dt_, double df_,
     ...
     *  Throws (CPU) or returns -1 (GPU) if @p t is out of [t0, t0 + num_times*dt]. */
    CUDA_DEVICE
    int get_time_index(double t);

    /** @brief Convert a physical frequency @p f [Hz] to its zero-based grid index.
     *  Throws (CPU) or returns -1 (GPU) if @p f is out of [f_min, f_max]. */
    CUDA_DEVICE
     ...
     *  Layout: invC[(noise_index * num_channels + channel) * num_times * num_freqs
     *               + t_idx * num_freqs + f_idx] */
    CUDA_DEVICE
    ...
    /** @brief Compute the flat index into the full (3x3) inverse-covariance matrix array.
     *  Layout: invC[((noise_index * num_channels + ch_i) * num_channels + ch_j)
     *               * num_times * num_freqs + t_idx * num_freqs + f_idx] */
    CUDA_DEVICE
    int get_noise_index_cross(int t_idx, int f_idx, int ch_i, int ch_j, int noise_index);

    /** @brief Return the off-diagonal inverse-covariance element C^{-1}_{ch_i, ch_j}
     *  at position (t_idx, f_idx) for the given noise instance. */
    CUDA_DEVICE
    cmplx get_invC_cross_value(int t_idx, int f_idx, int ch_i, int ch_j, int noise_index);
     ...
     */
    CUDA_DEVICE
    void get_d_d_inner_product_cross(
        cmplx *d_d,
        int t_idx, int f_idx,
        int channel_i, int channel_j,
        int data_index, int noise_index);
     ...
     */
    CUDA_DEVICE
    void get_d_d_inner_product_diag(
        cmplx *d_d,
        int t_idx, int f_idx,
        int channel,
        int data_index, int noise_index);
    ...
    /** @brief Dispatcher: accumulate (d|h) and (h|h) contributions at one (t,f) pixel.
     *
     ...
     * @param template_vals_add     Template for the proposed source, length num_channels
     * @param template_vals_remove  Template for the source being removed, length num_channels
     */
    CUDA_DEVICE
    void add_ip_swap_contrib(
     ...
     *    2. like_sum_from_contrib_cmplx – final reduction across blocks per binary.
     *  On CPU a single serial loop replaces both kernels.
     *
     ...
     * @param start_times_all    Physical start time for each binary's sub-grid [s]
     * @param start_freqs_all    Physical start frequency for each binary's sub-grid [Hz]
     ...
     * @param n_f_template       Number of frequency bins in each template sub-grid
     */
    void compute_likelihood_terms_wrap(
        cmplx *d_h_out, 
        cmplx *h_h_out,
        cmplx *template_vals,
...
public:
    CUDA_CALLABLE_MEMBER
    FDDomain(int num_freqs_, int num_channels_,
             double f_min_, double f_max_, double df_,
             cmplx *data_, cmplx *invC_,
             int num_data_, int num_noise_, int tdi_type_)
...
public:
    ...
    CUDA_DEVICE
    cmplx get_fourier_value(double amp, double phase0, double f0, double fdot0, double t0, double f, double window_factor);
 ...
 *
 * @param d_h_contrib   Partial sums for (d|h), shape [num_binaries * num_blocks_x]
 * @param h_h_contrib   Partial sums for (h|h), shape [num_binaries * num_blocks_x]
 ...
 * @param start_freqs_all Physical start frequency of each template sub-grid [Hz]
...
/** @brief Second-pass kernel: reduce per-block partial sums to per-binary results.
 *
 ...
 * @param d_h_contrib      Input partial sums, shape [num_binaries * num_blocks_per_bin]
 * @param h_h_contrib      Input partial sums, shape [num_binaries * num_blocks_per_bin]
