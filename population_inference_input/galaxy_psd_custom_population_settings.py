"""Run with: uv run python /workspace/rrondeel/pop_inf/LISAanalysistools/scripts/run_global.py -sfp /workspace/rrondeel/pop_inf/LISAanalysistools/population_inference_input/galaxy_psd_custom_populations_settings.py """
...
def setup_recipe(
    ...
    iteratively_resolved_population_path = "/workspace/rrondeel/pop_inf/data/iteratively_resolved_gbs_0.75yrs_snr7_estnoise_weak_int.npy"
...
from ..src.lisatools.globalfit.stock.erebor import HyperSetup, HyperSettings

def get_hyper_erebor_settings(general_set: GeneralSetup) -> HyperSetup:
    
    prior_file = "/workspace/rrondeel/pop_inf/LISAanalysistools/src/lisatools/globalfit/prior_files/population_priors/model.prior"
    
    catalog_weak_int = np.load("/workspace/rrondeel/pop_inf/data/catalogue_dwds_with_weak_interaction_gbgpu.npy")
    catalog_strong_int = np.load("/workspace/rrondeel/pop_inf/data/catalogue_dwds_with_strong_interaction_gbgpu.npy")
...
def get_general_erebor_settings() -> GeneralSetup:
    ...
    file_store_dir = head_dir + "dataloader_testing/"
