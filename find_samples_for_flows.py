...
class UpdateNewResiduals(Update):
    def __init__(self, fd, fp_residuals, fp_psd, burn=1000):
        self.fp_residuals = fp_residuals
        self.fp_psd = fp_psd
    ...
    def __call__(self, iter, last_sample, sampler):
        ...
        breakpoint()
        like_prep = sampler.log_prob_fn
        data_minus_templates_A, data_minus_templates_E = np.load(self.fp_residuals)[0, 0], np.load(self.fp_residuals)[0, 1]

        psd_A, psd_E = np.load(self.fp_psd)[0, 0], np.load(self.fp_psd)[0, 1]
        
        like_prep.psd[0] = xp.asarray(psd_A)
        like_prep.psd[1] = xp.asarray(psd_E)
        like_prep.injection_channels[0] = xp.asarray(data_minus_templates_A)
        like_prep.injection_channels[1] = xp.asarray(data_minus_templates_E)
        ...
        lp = last_sample.log_prior
        ll = sampler.compute_log_like(last_sample.branches_coords, logp=lp)
        last_sample.log_like = ll

        new_last_sample = sampler.sample(last_sample, iterations=self.burn, tune=False, skip_initial_state_check=True, thin_by=1, store=False, progress=False)

        breakpoint()


def run_single_band_search():
    # process_i, gpu_i, sub_band_i, nwalkers_prep, ntemps_prep, sub_band_lower_f0, sub_band_upper_f0, generating_priors, Tobs, oversample, waveform_kwargs, data_minus_templates
    ...
    residual_file = "residuals_for_flows.npy"
    psd_file = "psds_for_flows.npy"
    freq_file = "freq_for_flows.npy"

    update = UpdateNewResiduals(fd,residual_file, psd_file, burn=1000)

    data_minus_templates_A, data_minus_templates_E = np.load(residual_file)[0, 0], np.load(residual_file)[0, 1]

    psd_A, psd_E = np.load(psd_file)[0, 0], np.load(psd_file)[0, 1]

    # fd = np.load(freq_file)
    ...
    like_prep.inject_signal(
        data_stream=[data_minus_templates_A, data_minus_templates_E],
    ...
    sub_band_i = 400
    sub_band_lower_f0 = search_f_bin_lims[sub_band_i] / 1e3
    sub_band_upper_f0 = search_f_bin_lims[sub_band_i + 1] / 1e3
    ...
    prep_state = State({"gb": start_coords}, log_like=start_ll)
    ...
    fp_band = f"samples_for_flows_band_{sub_band_i}.h5"
    ...
    out = sampler_prep.run_mcmc(prep_state, nsteps_prep, burn=1000, progress=True, thin_by=100)
    breakpoint()
    # print(f"end - process: {process_i}, sub band: {sub_band_i}, gpu: {gpu_i}")
    lp = sampler_prep.get_log_like()
    ...
    inds = np.argsort(out.log_like.flatten())[::-1]

    num_coords_out = 500
    starting_coords = out.branches["gb"].coords.reshape(-1, 8)[inds[:num_coords_out]]
    starting_ll_from_sampler = np.sort(out.log_like.flatten())[::-1][:num_coords_out]
