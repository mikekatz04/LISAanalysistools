...
def run_gb_bulk_search():
    ...
    temperature_control = TemperatureControl(ndim, nwalkers, ntemps=10)
    ...
    for iter_i in range(iters):
        for evens_odds in range(2):
            ...
            keep_binaries = gb.d_h / xp.sqrt(gb.h_h.real) > 5.0
            ...
            old_points_with_fs = old_points.copy()
            old_points_with_fs[:, :, :, 1] = (f0_maxs[band_inds_here] - f0_mins[band_inds_here]) * old_points_with_fs[:, :, :, 1] + f0_mins[band_inds_here]
            old_points_with_fs[:, :, :, 2] = (fdot_maxs[band_inds_here] - fdot_mins[band_inds_here]) * old_points_with_fs[:, :, :, 2] + fdot_mins[band_inds_here]

            starting_points = old_points_with_fs[0, :, keep_binaries].get()
            ...
            print(iter_i, f"Number of bands running: {band_inds_running.sum().item()}, found {num_binaries_found_this_iteration} binaries. Total binaries: {binaries_found.shape[0]}")

            nwalkers_pe = nwalkers
            ntemps_pe = 6
            fp_gb_mixing = "mixing_gb_run.oickle"
            ...
            inds_new = np.ones((ntemps_pe, nwalkers_pe, nleaves_max_fix_new), dtype=bool) 
            coords_new[:, :, nleaves_max_fix:] = starting_points.transpose(1, 0, 2)[None, :]
            ...
            mixing_steps = 10
            save_every_steps = 20
            for mix_step in range(mixing_steps):
            ...
            best_mapped_ind = mgh.get_mapped_indices(best_logl_ind_after_mix)
            breakpoint()
            A_best_residuals = mgh.data_shaped[0][0][best_mapped_ind]
            E_best_residuals = mgh.data_shaped[1][0][best_mapped_ind]

            A_best_psd = mgh.psd_shaped[0][0][best_mapped_ind]
            E_best_psd = mgh.psd_shaped[1][0][best_mapped_ind]
