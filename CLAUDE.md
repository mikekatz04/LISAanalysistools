...
- `binding.cxx` / `binding.hpp` — pybind11 module exposing the C++/CUDA functions to Python. `binding.cxx`'s `PYBIND11_MODULE(pycppdetector, m)` body calls `detector_part(m)` then `response_part(m)` so OrbitsWrap, LISAResponseWrap, TDIConfigWrap all register here. (CubicSplineWrap moved to GBT's `interp` module 2026-06-10 — GBT is its single registrant.)
