"""Verify the MOVED + frame-fixed lisatools.sources.emri.EMRITDIonFly works vs the
legacy pyResponse on the SAME ecliptic orbit/window/data. Uses the actual package
class (not an inline copy), all-ecliptic frame.
"""
...
from lisatools.sources.emri import EMRITDIonFly                       # the moved class
...
def main():
    ...
    print(f"  EMRITDIonFly (package, all-ecliptic), thresh={THRESH:.0e}...", flush=True)
    fly = EMRITDIonFly(wg, orb, tdi_config, DT, TOBS_S, REF)
    out = fly(*inj, include_minus_mkn=True, mode_selection_threshold=THRESH)
    ...
    print(f"  inside {int(inside.sum())}/{N_WIN}", flush=True)

    print(f"  LEGACY pyResponse (ecliptic), thresh={THRESH:.0e}...", flush=True)
    ...
    for tag, sig in [("EMRITDIonFly", tof), ("legacy", leg)]:
        opt, _ = ac.template_snr(TDSignal(sig, td_set).transform(fd_set, window=win))
        print(f"    {tag:13s} opt SNR={float(opt):.3f}  opt/data={float(opt)/np.sqrt(dd):.3f}", flush=True)
    ...
    for ci, ch in enumerate("XYZ"):
        ...
        print(f"    |O|(EMRITDIonFly,legacy) {ch}={O:.4f}", flush=True)
