"""mm between TDI-on-the-fly (B) and legacy pyResponse (A) vs orbit-position
grid POS_DT.  Same waveform, params, t_ref fix -- only the response differs.
If A-vs-B shrinks with finer POS_DT, the 300 s position coarsening (added for
memory) is what kept the two responses from agreeing to machine precision.
Set MBH_POS_DT to sweep.
"""
import os, sys, gc, time, threading, resource
os.environ.setdefault("OMP_NUM_THREADS", "8")
import numpy as np
MEM_CAP_GB = float(os.environ.get("MBH_MEM_CAP_GB", "7.5")); _IS_MAC = sys.platform == "darwin"
def rss_gb():
...
TUKEY_ALPHA = 0.05; DTMIN = 0.1
POS_DT = float(os.environ.get("MBH_POS_DT", "300.0"))
WIN_DAYS = float(os.environ.get("MBH_WIN_DAYS", "6.0"))
DUR_DAYS = float(os.environ.get("MBH_DUR_DAYS", "5.0"))
INC = float(os.environ.get("MBH_INC", "0.4155"))
...
def main():
    ...
    banner(f"A(legacy) vs B(on-the-fly)  POS_DT={POS_DT}s  win={WIN_DAYS}d inc={INC:.4f}")
    ...
    del lt; gc.collect(); orb.configure(linear_interp_setup=True, dt=POS_DT); mark(f"orbit ready (pos_dt={POS_DT})")
    ...
    def mm(lo, hi):
        ...
        a = AnalysisContainer(TDSignal(A, td).transform(fd, window=win), XYZ2SensitivityMatrix(fd, model=SENS_MODEL))
        return 1 - abs(a.template_inner_product(TDSignal(B, td).transform(fd, window=win), normalize=True, complex=True))
    banner(f"A-vs-B mismatch  (POS_DT={POS_DT}s)")
    for tag, l, h in [("full", F_MIN, F_MAX), (">5e-4", 5e-4, F_MAX), (">1mHz", 1e-3, F_MAX), (">2mHz", 2e-3, F_MAX)]:
        print(f"  {tag:7s} {mm(l, h):.4e}", flush=True)
    print("\nDONE.", flush=True)
