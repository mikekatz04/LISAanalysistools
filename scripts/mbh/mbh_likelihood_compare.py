"""lisatools AnalysisContainer likelihood / inner-product comparison of the MBH
templates against the mojito data, for sensitivities scirdv1 and mrdv1.

  A = legacy pyResponse (PhenomTHMTDIWaveform)
  B = TDI-on-the-fly (MBHTDIonFly, t_ref fix + coarse_graining_scale_factor=48)

Reports per (template, sensitivity, band):  <d|d>, <d|h>, <h|h>, SNR_data,
SNR_opt, SNR_det, logL = -1/2 <d-h|d-h>, and mismatch 1-|O|.
"""
...
def main():
    ...
    banner("lisatools likelihood / inner-product vs mojito data  (scirdv1 + mrdv1)")
    ...
    print(f"  N_WIN={N_WIN} inc={inc:.4f} dur={dur_s/86400:.1f}d scale_factor={SCALE}", flush=True)
    ...
    mbh = MBHTDIonFly(wg, orb, TDIConfig(TDI_GEN_STR, force_backend=BACKEND), DT, dur_s,
    ...
    # Persist the TD-domain TDI arrays so the likelihood can be recomputed by a
    # process importing ONLY the lisatools likelihood machinery (no waveform /
    # response generators) -- see scripts/sobbh/likelihood_pure.py.
    arr_cache = f"/tmp/mbh_ll_arrays_id{MBHB_ID}.npz"
    np.savez(arr_cache, D=D, A=A, B=B, dt=DT,
    ...
    for name, T in [("A = legacy pyResponse", A), ("B = TDI-on-the-fly (scale=48)", B)]:
        banner(name + "   vs mojito data")
        print(f"  {'sens':>8} {'band':>17} {'<d|d>':>11} {'<d|h>':>11} {'<h|h>':>11} "
              f"{'SNRd':>7} {'SNRopt':>7} {'SNRdet':>8} {'logL':>11} {'mm':>10}", flush=True)
        for sens in SENS:
            for tag, flo, fhi in bands:
                ...
                dh = ac.template_inner_product(t, normalize=False, complex=False)
                dh = float(np.real(dh))
                opt, det = ac.template_snr(t)            # opt=sqrt(<h|h>), det=<d|h>/sqrt(<h|h>)
                hh = float(opt) ** 2
                logL = float(np.real(ac.template_likelihood(t)))
                O = ac.template_inner_product(t, normalize=True, complex=True)
                print(f"  {sens:>8} {tag:>17} {dd:11.3e} {dh:11.3e} {hh:11.3e} "
                      f"{np.sqrt(dd):7.1f} {float(opt):7.1f} {float(det):+8.2f} {logL:11.3e} {1-abs(O):10.3e}",
                      flush=True)
    print("\nDONE.", flush=True)
