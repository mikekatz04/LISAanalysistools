...
# ---- fill these in ---------------------------------------------------------
#SBATCH --job-name=gf_dense2_v7      # job name
...
#SBATCH --output=/shared/data/global_fit_output/gf_dense2_v7_%j.log     # combined stdout+stderr (captures [MAXLOGL]/[BENCH])
# ----------------------------------------------------------------------------
...
STORE_DIR=/shared/data/global_fit_output/gf_prod_3mo_dense2_v7/
...
# ############################################################################
# ## DENSE-BAND V7 PROBE (user, 2026-08-25). Identical configuration to    ##
# ## submit_gf_3mo_v7.sh (v6 mirror + GB_CAP_OVERLAP_FRAC=0.25 +          ##
# ## GB_SEARCH_RJ_REPLACE=1, 2 GPUs) confined to the OLD dense probe's    ##
# ## band (submit_gf_dense_probe.sh, layer-snapped values verbatim):      ##
# ## WDM layers 44-48 around band 41. The window holds 116 catalogue      ##
# ## sources, 69 detectable (SNR>7), closest real pair 2.3 FD bins --     ##
# ## the OVER-SUPPRESSION CONTROL for the overlap experiment: at 45-bin   ##
# ## covering cells that pair SHARES cells, so cap growth must            ##
# ## accommodate genuine neighbors here while the high-f side must stay   ##
# ## clean. Judge: detectable-count recovery vs the 69, no missing        ##
# ## close-pair members, vs the same window in v6's final store.          ##
# ## START: FRESH DIR ONLY (mkdir gf_prod_3mo_dense_v7) -- the confined   ##
# ## band grid cannot resume any full-band store. Noise stages rerun.     ##
# ## Layer-snap trap (from the old dense script): values a hair below a   ##
# ## layer boundary LOSE that layer -- these sit at 43.5/48.5 layers and  ##
# ## snap OUTWARD-safe to 44..48.                                         ##
# ############################################################################
# DENSE-2 (user, 2026-08-25): the DENSEST 5-layer window in the band by
# detectable count, found by sliding the window across the full-band truth
# set: layers 21.5-26.5 = 2.9861-3.6806 mHz. Contents: 2,541 catalogue
# sources, 1,832 kappa-kept, 156 DETECTABLE (SNR>7); closest genuine pair
# 0.4 FD bins; 109/155 detectable neighbor pairs within one 45-bin
# covering cell, 41 within 11.25 bins. The maximal crowding stress test:
# cap growth under overlap must serve ~2 detectable sources per cap cell
# on average here. Everything else identical to the dense_v7 probe
# (= v7 config confined). FRESH DIR ONLY (mkdir gf_prod_3mo_dense2_v7).
export GB_MIN_FREQ=2.986111e-03     # 21.5 layers -> snaps to 22
export GB_MAX_FREQ=3.680556e-03     # 26.5 layers -> snaps to 26
