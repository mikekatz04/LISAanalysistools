#!/bin/bash
# ============================================================================
# PRODUCTION global fit -- 6-month Tobs (gb + vgb + psd + galfor, mojito)
...
#
# ############################################################################
# ## 6MO_V1 (2026-08-24) -- THE FIRST 6-MONTH RUN.                          ##
# ##                                                                        ##
# ## This is a LITERAL COPY of submit_gf_3mo_v6.sh (user ruling 2026-08-23: ##
# ## "copy submit_gf_3mo_v6.sh and re-derive only the Tobs-dependent        ##
# ## lines") -- v6 is the winning 3-mo configuration (+2112 over v4-final   ##
# ## at less than half the iterations with 32% fewer leaves; the v6 band    ##
# ## trio DIVISOR=8 / STRIDE=9 / CAP_DIVISOR=4 is Tobs-INVARIANT per the    ##
# ## 2026-08-23 band_geom study). The 3-mo history blocks below (V4/V5/V6)  ##
# ## are kept: they document WHY each inherited knob is set.                ##
# ##                                                                        ##
# ## THE ONLY RE-DERIVED (Tobs-dependent) LINES:                            ##
# ##   * TOBS_TARGET=15552000 (180 d; Nf 1440 x Nt 4320 x dt 2.5 -- exact   ##
# ##     snap, same layer_df = 1.389e-4 Hz, band/slab geometry unchanged).  ##
# ##   * SIGHET_NT_LAYER=120 (divides Nt=4320; stride 36 layers = 36 h =    ##
# ##     the SAME temporal density as the validated 3-mo grid).             ##
# ##   * GB_N_SUBBANDS=4096 per GPU (slot cost ~2x the 3-mo ~1 MB -> same   ##
# ##     resident bytes as 3-mo's 8192).                                    ##
# ##   * GB_INMODEL_SETUP_BATCH=512 (stash law is LINEAR in N_sparse_t:     ##
# ##     ~2x the 3-mo per-source stash -> half the 3-mo 1024 cap = same     ##
# ##     bytes; 2026-08-23 ruling pattern from the 1-yr 256 cap).           ##
# ##   * EDGE_CROP_WAVELETS=60 (constant-LAYER edge exclusion ruling;       ##
# ##     2.5 d/side = 2.8%% of Tobs here; fresh store so the domain-shape   ##
# ##     caveat doesn't apply).                                             ##
# ##   * VGB_CHIRP_MASS_BASIS=1 + fresh store (the 6-mo debut the basis     ##
# ##     was built for; no migration on a fresh store).                     ##
# ##   * Store/job naming, GB_NLEAVES_MAX=15000.                            ##
# ##                                                                        ##
# ## BEFORE-LAUNCH GATES (tracker: docs/6mo-run-prep.md):                   ##
# ##   1. F-stat refit reference (user ruling 2026-08-23): refit against    ##
# ##      the GB-FREE residual with the CURRENT fitted sensitivity.         ##
# ##      VERIFIED IN CODE 2026-08-24: already implemented + default ON     ##
# ##      (GB_FSTAT_GB_FREE=1, _gb_free_residual, 85b59671/203c35be).       ##
# ##      OPEN INSTEAD: the refit CADENCE never fires in production (all    ##
# ##      three runs stuck at epoch 0 despite GB_FSTAT_REFIT_EVERY=50) --   ##
# ##      root-cause + fix before this launch or refits stay dead here too. ##
# ##   2. WARM START from the 3-mo posterior (workstream B): uncomment the  ##
# ##      GB_WARM_START block below once the clustered posterior->proposal  ##
# ##      builder lands. Donor: v6 (else v5 <= it471).                      ##
# ##   3. MBHB/SOBHB/EMRI fold-in (user ruling 2026-08-23: sources ARE in   ##
# ##      scope) -- gated on the source-merge campaign (T3-gpu/T4-heavy/    ##
# ##      T5-mg at THIS Tobs). MBH selection: only systems that MERGE       ##
# ##      within the 6-mo window. Until the campaign is green this script   ##
# ##      runs the proven 4-branch configuration.                           ##
# ##   4. Sig-het SIGHET_N_CP=256 re-verification on the 6-mo grid          ##
# ##      (gb_sighet_bfold_gpu_probe.py) -- 256 is the arena ceiling and    ##
# ##      is COARSER effective spacing here than the 3-mo-validated one.    ##
# ##   5. Shave-stack knobs (2026-08-23 dive: temper cadence double-fire,   ##
# ##      PRELOAD_CELLS, audit cost, REFRESH_DPHASE) are NOT armed here --  ##
# ##      A/B them on the running 3-mo v6 first; commented block below.     ##
# ############################################################################
#
# ############################################################################
...
# ---- fill these in ---------------------------------------------------------
#SBATCH --job-name=gf6mo_v1          # job name
...
# FRESH RUN by necessity: the 6-mo Tobs changes the WDM grid (Nt 2160->4320),
...
mkdir -p ${STORE_DIR}
# FIRST-LAUNCH CHECK (2026-08-23): every current cluster launch is writing
# 0-BYTE gpu_util CSVs (sampler wrapper broken cluster-side since ~07:30
# Aug 23). VERIFY this file is growing a few minutes in -- the 6-mo memory
# margin questions have no answer without it.
...
# would silently re-grid this run. Pin the 180-d production value:
# 6 months = 1.5552e7 s snaps EXACTLY to Nf=1440 x Nt=4320 x dt=2.5 (Nt = 2x
# the 3-mo grid at the SAME layer_df = 1.389e-4 Hz -- band/slab geometry and
# the v6 band trio carry over unchanged).
export TOBS_TARGET=15552000
# Sig-het sparse time grid: 120 divides Nt=4320 -> stride 36 layers = 36 h,
# the SAME temporal density as the validated 3-mo production grid (which
# runs the default 64 -> snapped 60 at Nt 2160). Pinned so it lands exactly
# instead of snapping. (-1 = AUTO ~35 h prescription is the fallback.)
...
# 6-MO RE-DERIVATION: per-slot cost ~2x the 3-mo ~1 MB (data slab + XYZ
# invC scale with the layer's FD bins x Nt), so 4096/GPU holds the SAME
# resident bytes the 3-mo run proved at 8192/GPU. Check the honest sizing
# INFO line on first launch before raising it.
export GB_N_SUBBANDS=4096  # PER GPU; TRUE per-slot cost incl. XYZ invC (~1 MB @3mo, ~2 MB @6mo, ~8 MB @23mo) x 2 move caches -- job-183 sizing; total = x n_gpus
...
# 6-MO CAVEAT (before-launch gate 4): 256 nodes over 180 d is a COARSER
# effective spacing than the 0.35-day criterion verified at 3 months --
# re-verify with gb_sighet_bfold_gpu_probe.py ON THIS GRID before trusting
# low-f h_h; the durable long-Tobs fix is the amp/phase redesign
# (project_signal_het_amp_phase_redesign), not a node-count knob.
...
# knob (taper must be subsumed; constant-layers scaling) -- and THIS run
# is the fresh store it was waiting for. 60 layers = 2.5 d/side covers the
# old 54-layer taper region; relative cost 2.8%% at 6 mo (was 5.6%% at 3 mo).
# CAUTION (user, 2026-08-24, VERIFICATION PENDING): the constant-layer
# ruling assumed the crop tracks the WDM window scale only -- but
# SIGHET_TUKEY_ALPHA=0.01 is a FRACTION of the record, so the taper ramp
# LENGTHENS with Tobs (~11 layers/side at 3 mo -> ~22 at 6 mo at fixed
# alpha). The crop must stay >= taper + margin as Tobs grows: 60 looks
# sufficient at 6 mo (~22+8=30) but the LAW is crop >= f(alpha x Nt) --
# re-derive per Tobs (23-mo at alpha 0.01 would NOT fit in 60), or scale
# alpha down instead (equal-alphas policy: change ALL paths together).
...
# 6-MO RE-DERIVATION: the stash law is LINEAR in N_sparse_t
# ((3nch+4nch^2) x Nf_active x N_sparse_t x 16B per source) -- N_sparse_t
# doubles at 6 mo (120 vs 60), so 512 = the SAME staging bytes the 3-mo run
# proved at 1024. 1-yr runs 256 by the same law; do NOT raise this on a
# thin-margin card (2026-08-23 ruling: never trade the stash cap for speed).
...
# VGB CHIRP-MASS BASIS DEBUT (the 6-mo run this basis was BUILT for --
# 3-mo scripts deliberately stayed on the 5-dim distance basis while the
# likelihood fix was validated). Sampled [dist, phi0, cos_iota, psi, Mc,
# fdot_astro_ratio] (ndim 6): fdot = get_fdot(f0, Mc) * (1 + ratio); ratio
# init is ADDITIVE (VGB_RATIO_INIT_WIDTH default 0.02 -- the documented
# zero-truth exception to multiplicative START_FACTOR) so the stretch
# ensemble is never collapsed. At 6 months the fast chirpers' ratio
# posteriors should CONTRACT around 0 -- the built-in GR-consistency gate.
# FRESH STORE => NO migration needed (migrate_vgb_chirp_basis.py is only
# for carrying an old store's VGB state forward).
export VGB_CHIRP_MASS_BASIS=1
# 8-rung ladder (user ruling 2026-08-15; fresh store builds it directly
# via make_ladder -- no re-rung migration on a fresh store).
export VGB_NTEMPS=8
...
# ============================================================================
# WARM START (PENDING -- workstream B in docs/6mo-run-prep.md). Uncomment
# once the clustered posterior->proposal builder lands (design:
# docs/warm-start-gb-proposal.md; prototype scripts/gb/
# proto_warmstart_cluster.py). Components fitted OFFLINE (CPU, minutes)
# from the 3-mo donor store's cold-chain leaf samples; refit against the
# donor's FINAL store right before this launch.
# export GB_WARM_START_COMPONENTS=/path/to/gf_prod_3mo_warmstart.npz
# export GB_WARM_START_WEIGHT=...   # mixture weight vs the F-stat proposal
# ============================================================================
# SOURCE BRANCHES (PENDING -- the source-merge campaign, docs/6mo-run-prep.md
# workstream C; user ruling 2026-08-23: MBHB/SOBHB/EMRI ARE in scope for
# 6 mo). Until the campaign's T3-gpu/T4-heavy/T5-mg gates are green at THIS
# Tobs, this script runs the proven 4-branch configuration (the staged
# runner remove_branch's the others). When they fold in:
#   * MBH: ONLY systems that MERGE inside the 6-mo window (user ruling) --
#     selection via MBHB_IDS from a t_merge <= TOBS_TARGET catalogue cut.
#   * SOBBH: most are UNDETECTABLE at 6 mo (user expectation) -- census
#     first; chunked moving-window chirp build is the open blocker.
#   * EMRI: detectability at 6 mo unknown -- census first; the
#     interpolate.cu cusparse hard-exit hardening MUST be committed +
#     rebuilt on the cluster before any long run (process-killer).
# export MBHB_IDS=...   # empty drops the branch (NB SOBHB= ids spelling)
# export SOBHB_IDS=...
# export EMRI_IDS=...
# ============================================================================
# SHAVE STACK (2026-08-23 dive; NOT armed -- A/B on the running 3-mo v6
# first, then copy the verdict here): GB_TEMPER_EVERY_PROPOSES=30 (cadence
# double-fire fix, ~75 s/it), GB_TEMPER_PRELOAD_CELLS=2400 (+1.5 GB,
# ~55 s/firing -- MEMORY risk, telemetry currently dead), audit knobs
# ANCHOR/DRIFT_CHECK -> 0 after the first iterations (~25 s/PE call),
# GB_SIGHET_REFRESH_DPHASE=0.5 (offenders-only refresh).
# ============================================================================
...
# NO NOISE GRAFT AT 6 MO: the 3-mo graft shortcut (graft_noise_state.py)
# does NOT apply here -- the WDM grid changed (Nt 2160 -> 4320), so the
# noise stages MUST fit fresh on this grid (~1.5-3 h, once).

# LATER REFITS: GB_FSTAT_REFIT_EVERY=50 proposal-hits (~8 h at the new
