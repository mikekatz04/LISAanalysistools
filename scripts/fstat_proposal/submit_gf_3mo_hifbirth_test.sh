...
# ---- fill these in ---------------------------------------------------------
# ============================================================================
# HIGH-F BIRTH-FIX VERIFICATION RUN (2026-08-20, user directive).
# Byte-identical to submit_gf_3mo_v4.sh except STORE_DIR / job name /
# NUM_ITERATIONS=100. Purpose: verify LAT 1274a66c (tight fdot_astro_ratio
# birth proposal, default ON) against the measured pre-fix baseline at the
# 20.38 mHz source (band 142): pre-fix, only 4/24 cold walkers assembled a
# near-single template in ~59 iterations (the ~1%-per-birth fdot lottery);
# post-fix the F-stat grid's Mc -> fdot survives birth, so the majority of
# walkers should hold a single template with fdot ~ +1.0e-13 within ~10-20
# GB iterations, and the mosaic tail should shrink.
#
# SETUP (a COPY of a recent v4 store, rewound to GB-search start):
#   cp -r gf_prod_3mo_v4 gf_hifbirth_test          # or a saved copy from yesterday
#   rm -f gf_hifbirth_test/*CORRUPT*.h5
#   python scripts/fstat_proposal/reset_recipe_stage.py \
#       gf_hifbirth_test/gf_prod_3mo_testing.h5 gb_search --rewind-to-empty gb --apply
#   cp gf_hifbirth_test/gf_prod_3mo_testing.h5 \
#      gf_hifbirth_test/gf_prod_3mo_testing_running_backup_copy.h5
#   sbatch scripts/fstat_proposal/submit_gf_3mo_hifbirth_test.sh
#
# IN-RUN confirmation the fix is live: the epoch-0 grid build logs
#   "[birth] fdot_astro_ratio proposal TIGHTENED: ..."
# VERDICT data: gf_store_extract.py the store after ~30+ iterations; the
# analysis compares band-142 per-walker census + brightest-leaf fdot
# against the archived pre-fix numbers.
# ============================================================================
#SBATCH --job-name=gf3mo_hifb          # job name
...
#SBATCH --output=gf3mo_hifb_%j.log     # combined stdout+stderr (captures [MAXLOGL]/[BENCH])
# ----------------------------------------------------------------------------
