#!/bin/bash
# ============================================================================
# v9 = 3mo_v8 + ONE change: the RESEED+REPLACE combined proposal (user design
# 2026-09-04). Exact copy of submit_gf_3mo_v8.sh except the naming, and the
# reseed block (search rj_replace RE-ENABLED as host, its draw made IDENTICAL
# to the fstat search birth via GB_REPLACE_INCELL=uniform + phase-max only,
# under GB_RJ_FSTAT_DIST_BIRTH=0 which retires centering for both; plus
# GB_COLD_RESEED_EVERY=7 + GB_COLD_RESEED_REPLACE_PASSES=3 -- see the
# "v9 RESEED+REPLACE" block below, ~L1250). Every 7 iterations the replace
# move does multi-pass replace on temps<hottest, a grandfathered cold->hot
# reseed on the hottest rung, in-model + vertical swaps, then a forced fancy
# swap; on the other 6 it is a no-op. Fresh store (gf_prod_3mo_v9). OPEN RISK:
# rj_replace's lnL accounting is the same one v8 disabled (see the block).
# ============================================================================
...
# ##                                                                        ##
# ## An old gf_prod_3mo_v9/ store exists from the 2026-08-30 attempt. It    ##
...
# ##      mv /shared/data/global_fit_output/gf_prod_3mo_v9 \
# ##         /shared/data/global_fit_output/gf_prod_3mo_v9_aug30_attempt    ##
...
# ##                                                                        ##
# ##   S=/shared/data/global_fit_output/gf_prod_3mo_v9/gf_prod_3mo_testing.h5
# ##                                                                        ##
...
# ---- fill these in ---------------------------------------------------------
#SBATCH --job-name=gf_v9          # job name
...
#SBATCH --output=/shared/data/global_fit_output/gf_v9_%j.log     # combined stdout+stderr (captures [MAXLOGL]/[BENCH])
# ----------------------------------------------------------------------------
...
STORE_DIR=/shared/data/global_fit_output/gf_prod_3mo_v9/
...
SLURM_LOG=/shared/data/global_fit_output/gf_v9_${SLURM_JOB_ID:-manual}.log
LOG_MIRROR_PID=""
...
# OOM FIX inherited from v8 (2026-09-04): batch 4096->1024 + both mempool
# frees re-enabled, after the v8 run crashed at gb_tdi_on_the_fly.cu:3410
# (in-model sig-het reference buffer) ~iter 146. v9 ADDS the multi-pass
# replace (3x) which raises in-model/replace throughput per firing, so it
# is if anything MORE memory-hungry -- keep the conservative batch here.
...
# ===================================================================
# v9 RESEED+REPLACE COMBINED PROPOSAL (user design 2026-09-04).
# rj_replace is RE-ENABLED here as the HOST of the combined proposal, and
# its candidate DRAW is made IDENTICAL to the fstat search birth (user
# ruling 2026-09-04: "match fstat search EXACTLY / only maximize phase").
# HOW THAT WORKS: with GB_RJ_FSTAT_DIST_BIRTH=0 (below), the fstat SEARCH
# birth retires F-stat centering -- it draws intrinsics+extrinsics
# (iota/psi/amp) from the grid container and maximizes PHASE only. And
# _run_replace_step's ENTIRE F-stat-center block is guarded by
# `if self.rj_fstat_dist_birth` (gbspecialstretch.py:9219), so under =0 the
# replace ALSO draws its full candidate from that same container (cont.rvs)
# and phase-maximizes phi0 with rotation-on-accept -- no centering, no amp
# maximization (_replace_fstat_max needs rj_fstat_dist_birth, so it is
# already False). The ONE remaining draw divergence is the within-cell law:
# the replace defaults to trilinear, the birth uses the container's UNIFORM
# in-cell draw -- so pin it to uniform to match the birth exactly.
export GB_SEARCH_RJ_REPLACE=1
export GB_REPLACE_INCELL=uniform         # match the fstat birth in-cell draw
# GB_REPLACE_PHASE_MAX stays at its inherited =auto (below, ~L1531), which
# resolves ON for this search move (phase-max phi0 + rotation-on-accept) --
# same as the birth. GB_REPLACE_CTR_MODE is deliberately UNSET: its centering
# path is dead under GB_RJ_FSTAT_DIST_BIRTH=0 (guarded block above), and the
# birth uses no centering, so matching means NOT engaging it.
# The combined proposal itself (installed, default-off; see gbspecialstretch
# _reseed_host_armed / propose): every N iterations the replace move does
# multi-pass replace on temps<hottest, a grandfathered cold->hot reseed on
# the hottest rung, in-model + vertical swaps, then a forced fancy swap;
# on the other N-1 iterations the replace move is a no-op.
export GB_COLD_RESEED_EVERY=7            # firing cadence (0 = off = v8)
export GB_COLD_RESEED_REPLACE_PASSES=3   # replace sweeps per firing
# ===================================================================
