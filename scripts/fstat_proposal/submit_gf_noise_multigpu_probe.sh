#!/bin/bash
# ============================================================================
# MULTI-GPU PSD + GALFOR PROBE (2026-08-28) -- runs BEFORE the v8 production
# launch; "then we will mix it with the rest" (user).
#
# WHAT THIS IS: submit_gf_3mo_v8.sh, byte-identical in configuration, capped
# at NUM_ITERATIONS=40 in a DISPOSABLE store. The v8 staged recipe BEGINS
# with noise_search -- psd + galfor sampled jointly against the full mojito
# data with GB empty -- so simply running the head of the real script IS the
# isolated multi-GPU noise test, on exactly the configuration the production
# run will execute. Nothing to un-mix later: delete the probe store and
# launch the real script.
#
# WHAT IT ADDS ON TOP OF v8 (all BEFORE mpiexec, so failures cost seconds,
# not an allocation):
#   [PROBE-0] the cupy-gated unit tests un-skip on the GPU node
#             (unequal-arm CPU/GPU parity, coarse per-device statistics);
#   [PROBE-1] two-GPU unequal-arm basis smoke: per-device settings parity
#             for fold / layer_constant / layer_calibrated, PLUS the §3.3
#             regression -- ONE settings object + ONE shared basis_cache
#             entered under both devices must warm one entry PER DEVICE
#             with device-resident arrays (the fused_unit_covariances_v2
#             cache-key fix).
#
# WHAT TO READ AFTER (grep the slurm log / store):
#   [unequal-arm] link-delay table ...    stride/epochs/span/digest/data_t0
#   [galfor-modulation] ... anchored ...  table epoch + coverage
#   [noise-model-identity] {...}          the persisted resume identity
#   [GF_TIMING] ... move=psd... / galfor  noise-block walls (2-GPU target:
#                                         the ~43->9 s/it class win survives)
...
# ---- fill these in ---------------------------------------------------------
#SBATCH --job-name=gfnoiseprobe          # job name
...
#SBATCH --output=/shared/data/global_fit_output/gfnoiseprobe_%j.log     # combined stdout+stderr (captures [MAXLOGL]/[BENCH])
# ----------------------------------------------------------------------------
...
export NUM_ITERATIONS=40           # PROBE CAP: enough noise_search iterations for
                                   # walls + machinery verdicts; the probe is NOT
                                   # expected to finish the search stage.
...
# regression. Small grid; seconds. Aborts on any failure.
# ============================================================================
python - <<'PYEOF' || { echo "[PROBE-1] SMOKE FAILED -- aborting"; exit 2; }
import numpy as np
try:
    import cupy as cp
except ImportError:
    raise SystemExit("[SMOKE] cupy missing on this node")
ndev = cp.cuda.runtime.getDeviceCount()
print(f"[SMOKE] cuda devices visible: {ndev}")
...
Nt = 16
wob = 1e-4 * L0 * np.sin(
    2 * np.pi * np.arange(Nt)[:, None] / Nt + np.arange(6)[None, :]
)
base = L0 * (
...
ltts = base[None, :] + wob
model = LISAModel((15e-12) ** 2, (3e-15) ** 2, DefaultOrbits(), "probe")
cpu = WDMSettings(Nf=32, Nt=Nt, dt=5.0, force_backend="cpu")
failures = 0
# NOTE: layer_calibrated WARNS on this unrestricted toy grid (its band
# reaches Nyquist -- the documented degradation regime). EXPECTED here and
# harmless: the calibration ratio is host-resident on both sides, so the
# CPU/GPU parity below is unaffected. The production 3-mo band (MAX_FREQ
# 2.5e-2 vs 0.2 Hz Nyquist) is comfortably inside the safe regime and must
# NOT produce this warning in the run itself.
for method in ("fold", "layer_constant", "layer_calibrated"):
    ref = np.asarray(
        UnequalArmInstrumentNoise(
            ltts, model=model, fill_nans=0.0, basis_cache={},
            wdm_psd_method=method,
        ).covariance(cpu)
    )
    for dev in range(min(ndev, 2)):
        with cp.cuda.Device(dev):
            gset = WDMSettings(Nf=32, Nt=Nt, dt=5.0, force_backend="gpu")
            cov = UnequalArmInstrumentNoise(
                ltts, model=model, fill_nans=0.0, basis_cache={},
                wdm_psd_method=method,
            ).covariance(gset)
            dev_ok = isinstance(cov, cp.ndarray) and int(cov.device.id) == dev
            denom = np.maximum(np.abs(ref), 1e-300)
            rel = float(np.max(np.abs(cp.asnumpy(cov) - ref) / denom))
            val_ok = rel < 1e-10
            print(f"[SMOKE] method={method} dev={dev} device_ok={dev_ok} max_rel={rel:.3e}")
            failures += (not dev_ok) + (not val_ok)

# §3.3 regression: SAME settings + SAME shared cache under both devices ->
# one warmed entry PER DEVICE, second entry's arrays resident on device 1.
if ndev >= 2:
    with cp.cuda.Device(0):
        shared_settings = WDMSettings(Nf=32, Nt=Nt, dt=5.0, force_backend="gpu")
    shared_cache = {}
    for dev in (0, 1):
        with cp.cuda.Device(dev):
            UnequalArmInstrumentNoise(
                ltts, model=model, fill_nans=0.0, basis_cache=shared_cache,
                wdm_psd_method="layer_constant",
            ).covariance(shared_settings)
    entries = list(shared_cache.values())
    n_entries = len(entries)
    dev_ids = sorted({int(e[1].device.id) for e in entries})
    print(f"[SMOKE] shared-cache entries={n_entries} basis devices={dev_ids} (expect 2 entries, [0, 1])")
    if n_entries < 2 or dev_ids != [0, 1]:
        failures += 1
else:
    print("[SMOKE] single GPU node: shared-cache two-device regression SKIPPED")
raise SystemExit(1 if failures else 0)
PYEOF
echo "[PROBE-1] smoke OK"
