#!/bin/bash
# ############################################################################
# ## JOINT PSD + GALFOR RUN @ 2 YEARS (2026-09-05, user request).            ##
# ##                                                                        ##
# ## THE TWO NOISE FITS TOGETHER, on real data. Data = mojito's instrument   ##
# ## stream (data/INSTRUMENT/L1) SUMMED with its galactic-foreground stream  ##
# ## (data/GALFOR/L1). Fit = psd (2 params Soms_d, Sa_a) + galfor (5 params  ##
# ## amp, fk, alpha, f_1, f_2), each on its own tempering ladder.            ##
# ##                                                                        ##
# ## ⚠ READ THE BRICK'S PROVENANCE BEFORE READING THE RESULT. The GALFOR     ##
# ## brick's own attrs say: derived_from the GB brick, "GalacticStochastic   ##
# ## resolvable binaries subtracted ... include_undecided=False,             ##
# ## include_above_band=True". It is the REAL unresolved-GB confusion        ##
# ## residual -- NOT a draw from the analytic tanh foreground the galfor     ##
# ## branch fits. Consequences for the readout:                              ##
# ##   * psd_injection (Soms_d 1.5e-11, Sa_a 3e-15) IS truth. Whether the    ##
# ##     INSTRUMENT PSD stays unbiased while galfor absorbs the confusion    ##
# ##     is the headline -- this is the clean 2-branch version of the ~1.4x  ##
# ##     PSD bias (S_oms 1.39x, S_tm 1.63x) measured in the full 3-mo v8 GB  ##
# ##     run. The 09-04 normalization audit ruled out a likelihood/          ##
# ##     normalization bug and pinned the bias on exactly this confusion.    ##
# ##   * galfor_injection is a REFERENCE CURVE, NOT truth. There is no true  ##
# ##     (amp, fk, alpha, f_1, f_2) for a real GB residual, so a galfor      ##
# ##     parameter offset is a MODEL-ADEQUACY statement, not a recovery      ##
# ##     failure. Do not quote "galfor recovered X% off" as an error.        ##
# ##                                                                        ##
# ## THE DECISIVE OBSERVABLE: does alpha rail at GALFOR_ALPHA_MAX? In 3-mo   ##
# ## v8, alpha railed at the stock cap 5.0 (60% of cold samples within 2% of ##
# ## the edge) while amp sat mid-prior with 4 decades of headroom -- i.e.    ##
# ## the foreground wants a STEEPER SHAPE than the cap allows. This script   ##
# ## runs the widened cap 20.0 (matching the shipped 3-mo diagnostic), so:   ##
# ##   alpha settles interior + PSD unbiased -> the tanh form is adequate    ##
# ##     and the 1.4x was the alpha cap;                                     ##
# ##   alpha re-rails at 20 -> the tanh FORM is too rigid for the real       ##
# ##     confusion and needs replacing (not just a wider prior);             ##
# ##   alpha interior but PSD still biased -> the bias is NOT the foreground ##
# ##     model at all and the search moves elsewhere.                        ##
# ##                                                                        ##
# ## CONTROL: gf_noise_2yr (submit_gf_noise_2yr.sh) -- the SAME grid, model  ##
# ## and scoring with the GALFOR stream and the galfor branch removed. Its   ##
# ## psd posterior is the unbiased reference this run is measured against.   ##
# ## Keep both stores; do NOT resubmit this script into that one.            ##
# ##                                                                        ##
# ## CHANGED vs submit_gf_noise_2yr.sh (the whole diff):                     ##
# ##   driver      --stock noise_mojito -> --stock noise_galfor_mojito       ##
# ##   naming      gfnoise_2yr/gf_noise_2yr -> gfnsgal_2yr/gf_noise_galfor_2yr
# ##               (fresh store MANDATORY: the BRANCH SET changes, and a     ##
# ##                resume across that dies with a bare KeyError deep in     ##
# ##                GFState.__init__, not a clear message)                   ##
# ##   + GALFOR_ALPHA_MAX=20.0 (the shape diagnostic above)                  ##
# ##   + the GALFOR brick preflight                                          ##
# ## Tobs, the WDM grid, the v8 noise model, the exact-fine coarse block and ##
# ## the sampler shape are all byte-identical to the control.                ##
# ##                                                                        ##
# ## REQUIRES (same commit): the L1DataLoader GALFOR stream type and the     ##
# ## noise_galfor_mojito variant. GALFOR is a whole-STREAM source type like  ##
# ## NOISE (no catalogue, no source ids, no source{id}_ infix in the brick   ##
# ## name) -- it is summed into the data, not loaded as a source class.      ##
# ##                                                                        ##
# ## WALKER FLOOR: galfor is ndim 5 on a plain stretch (no                   ##
# ## live_dangerously), so eryn refuses nwalkers < 2*ndim = 10. NWALKERS=24  ##
# ## below is fine; do not lower it below 10.                                ##
# ##                                                                        ##
# ## LAUNCH: git pull && sbatch scripts/fstat_proposal/submit_gf_noise_galfor_2yr.sh
# ############################################################################
# ============================================================================
# INHERITED 2-YEAR BLOCK (from submit_gf_noise_2yr.sh). Its Tobs/grid content
# is LIVE and applies verbatim; only its "noise-only / psd-only / no galfor"
# statements are superseded by the joint block above.
# ============================================================================
# ############################################################################
# ## NOISE-ONLY PSD VERIFICATION RUN @ 2 YEARS (2026-09-05, user request).   ##
# ##                                                                        ##
# ## This is submit_gf_noise_v8.sh with ONE variable moved: Tobs 90 d ->     ##
# ## 720 d. Every other line -- the v8 noise model (UNEQUAL_ARM=1,           ##
# ## WDM_PSD_METHOD=layer_calibrated, tabulated modulation anchored at       ##
# ## data_t0, fiducial=injection), the exact-fine coarse block, the sampler  ##
# ## shape, the (inert) GB knobs -- is BYTE-IDENTICAL to it, so the 90-d     ##
# ## result (job 436) is a clean control for this one.                       ##
# ##                                                                        ##
# ## WHAT THE 90-d RUN ESTABLISHED (job 436, snapshot 3): the PSD model      ##
# ## recovers the injection to ~1% on pure instrument noise -- cold          ##
# ## S_oms = 1.50117e-11 vs 1.5e-11 injected (1.0008x, +/-0.07%),            ##
# ## S_tm = 2.966e-15 vs 3e-15 (0.989x, +/-0.3%). That is what pinned the    ##
# ## ~1.4x instrument-noise bias in the full v8 GB run on the UNMODELED      ##
# ## galaxy + unresolved GBs rather than on the PSD model. THIS run asks     ##
# ## the same question at the production baseline: does the model still      ##
# ## recover the injection once the unequal-arm breathing and the tabulated  ##
# ## modulation have 8x as long to accumulate, on the grid the 2-yr          ##
# ## production fit will actually use?                                       ##
# ##                                                                        ##
# ## CHANGED vs submit_gf_noise_v8.sh (the whole diff):                      ##
# ##   TOBS_TARGET  7776000 -> 62208000     (90 d -> 720 d; see that block)  ##
# ##   job-name / --output / STORE_DIR / SLURM_LOG / BASE_FILE_NAME          ##
# ##       gfnoise_v8 -> gfnoise_2yr, gf_noise -> gf_noise_2yr               ##
# ##       (a NEW store: a resume across a Tobs change is a different array  ##
# ##        shape, not a resume -- the 90-d store must stay untouched, it    ##
# ##        is the control and its readout is still being cited)             ##
# ##   + the Tobs-vs-table-coverage preflight at the bottom                  ##
# ##                                                                        ##
# ## REQUIRES the run_global.py TOBS_TARGET fix landed in the same commit    ##
# ## -- without it this script SILENTLY RUNS 90 d. See the TOBS_TARGET       ##
# ## block for the mechanism and the log line that proves it took.           ##
# ##                                                                        ##
# ## COST at 8x the pixels (extrapolated from job 436's 3.7 s/it, 24w,       ##
# ## 3.6 GB GPU; the psd branch is 2 params and the likelihood is exact-fine ##
# ## so both scale ~linearly in Nf*Nt): ~30 s/it and ~29 GB/card. 2000       ##
# ## iterations ~ 17 h, inside the 24 h wall, and the run is resume-safe on  ##
# ## a spot preemption -- resubmit the same script.                          ##
# ##                                                                        ##
# ## NOT CHANGED, DELIBERATELY:                                              ##
# ##  * COARSE_Q=1 / COARSE_GPU_MODE=off. Not a choice -- the noise-only     ##
# ##    variant HARD-REJECTS the GPU coarse sidecar and forbids Q>1 with     ##
# ##    GPUs set (job 432 crashed on it). Exact-fine is also the honest      ##
# ##    scoring for a model-fidelity check. It does mean the 8x cost is      ##
# ##    paid in full.                                                        ##
# ##  * EDGE_CROP_WAVELETS (left unset -> the variant's 20). The GB scripts  ##
# ##    pin 60 from 6 mo up to subsume a 54-layer Tukey taper; this variant  ##
# ##    runs window_tukey_alpha=0.0 (RECTANGULAR -- no taper to subsume),    ##
# ##    so the crop only has to cover boundary wavelets, which is a FIXED    ##
# ##    layer count at any Tobs. 20 stays correct here and costs 2*20/17280  ##
# ##    of the data instead of 2*20/2160.                                    ##
# ##  * the GB_* / SIGHET_* / F-stat knobs below. Inert: no gb branch. They  ##
# ##    are kept byte-identical rather than "scaled to 2 yr" precisely so    ##
# ##    the diff against the 90-d control stays one line.                    ##
# ##                                                                        ##
# ## LAUNCH:  git pull && sbatch scripts/fstat_proposal/submit_gf_noise_2yr.sh
# ############################################################################
# ============================================================================
# INHERITED HEADER (from the 3-mo GB script this file descends through).
# It describes the GB/VGB/F-stat production run and is HISTORY ONLY here:
# this script runs --stock noise_galfor_mojito, which has psd + galfor and
# NO gb/vgb branch. The v8 NOISE block below it IS live -- that is the model
# under test. Read the JOINT block at the top for what this run actually is.
# ============================================================================
...
# ---- fill these in ---------------------------------------------------------
#SBATCH --job-name=gfnsgal_2yr         # job name
...
#SBATCH --output=/shared/data/global_fit_output/gfnsgal_2yr_%j.log     # combined stdout+stderr (captures [MAXLOGL]/[BENCH])
# ----------------------------------------------------------------------------
...
STORE_DIR=/shared/data/global_fit_output/gf_noise_galfor_2yr/
...
SLURM_LOG=/shared/data/global_fit_output/gfnsgal_2yr_${SLURM_JOB_ID:-manual}.log
LOG_MIRROR_PID=""
...
export BASE_FILE_NAME=gf_noise_galfor_2yr
...
# would silently re-grid this run. Pin the value explicitly.
#
# ***THE ONE VARIABLE OF THIS SCRIPT: 90 d -> 720 d (2026-09-05).***
#
# 62208000 s = 720 d, the exact FACTOR-2 of the 1-yr script's 31104000 and
# the 8x of the 3-mo 7776000 -- so the derived WDM grid is Nf 1440 x Nt
# 17280 at the SAME 3600 s layer width every script in this family runs
# (verified: derive_wdm_grid(62208000, dt=2.5, (3600,4400)) -> 1440 x 17280,
# Tobs 62208000 exactly; at 7776000 the same call returns 1440 x 2160, i.e.
# this run's ONLY grid change is Nt).
#
# WHY NOT 2 x YRSID (63116299.5 s = 730.5 d). The unequal-arm tables anchored
# at data_t0 cover 730.511 d exactly: scripts/noise/modulation_unequal.dat
# spans [9.7729940e7, 1.6084614e8] s absolute = 6.3116198e7 s of data frame,
# and the NOISE brick's /ltts table spans the same window. 2 x YRSID snaps to
# Nt 17532 = 730.500 d = 6.3115200e7 s -- INSIDE coverage by 997 s, i.e. less
# than ONE 3600 s wavelet layer of margin, on the table whose coverage check
# is a build-time ValueError (job 433 died on exactly that check). 720 d
# leaves 10.5 d of margin and matches the family's day accounting, where
# "1 yr" is already 360 d. The preflight below now checks this rather than
# leaving it to a comment.
#
# ⚠ TOBS_TARGET ALONE WAS NOT ENOUGH -- CODE FIX REQUIRED (2026-09-05).
# MojitoNoiseGeneralSettings PINS the WDM grid (nf=1440, nt=2160) and by the
# settings contract a fixed (nf, nt) BEATS general.tobs_target, so this
# export was inert on the --stock driver: the 90-d run only agreed with it
# by coincidence (1440*2160*2.5 s = 7776000 s). run_combined_staged.py has
# cleared nf/nt on a set TOBS_TARGET since 2026-08-13 (that fix is why the
# 23-mo shakedown stopped silently running 90 d); scripts/run_global.py --
# the driver THIS script uses -- never got it. Ported in the same commit as
# this file. WATCH FOR IT ON LAUNCH, it is the proof the re-grid took:
#   [stock] TOBS_TARGET=6.2208e+07 s: cleared any fixed-grid (nf, nt) ...
# and then the engine's own grid line reporting Nt=17280. If you see 2160,
# the driver is stale -- git pull.
export TOBS_TARGET=62208000
...
# ---- GALFOR SHAPE DIAGNOSTIC (the point of this run) -----------------------
# Stock alpha cap is 5.0 and the 3-mo v8 run RAILED against it (60% of cold
# samples within 2% of the edge; mean at 94% of the prior width) while amp sat
# mid-prior with ~4 decades of headroom -- the foreground wants a steeper
# SHAPE, not a different level. Capping alpha at 5 here would pre-bias the one
# question this run exists to answer, so widen to 20.0 (the value already
# shipped in submit_gf_3mo_v8.sh, so the two are comparable). Raises ONLY the
# alpha upper bound; every other prior edge is untouched, and a resume under a
# wider cap is safe (railed values stay in support). Unset -> stock 5.0.
export GALFOR_ALPHA_MAX=20.0
# galfor's own ladder (code default 12; pinned for the run record). The psd
# and galfor moves each carry an INDEPENDENT ladder -- that is what
# share_temperature_control=False in the recipe buys.
export GALFOR_NTEMPS=12
...
#   * (2-yr) the requested Tobs must FIT INSIDE the modulation table's span;
...
# ============================================================================
python - "${GALFOR_MODULATION_PATH}" "${MOJITO_DATA_PATH}" "${NOISE_FILE:-}"   "${STORE_DIR}/${BASE_FILE_NAME}_testing.h5" "${WDM_PSD_METHOD}" "${TOBS_TARGET}" <<'PYEOF' || exit 2
import glob, os, sys
import h5py
mod, mojito, noise_file, store, method, tobs = sys.argv[1:7]
tobs = float(tobs)
if not os.path.isfile(mod):
    print(f"[V8-NOISE] REFUSING: modulation table {mod!r} not found.")
    raise SystemExit(2)
# TOBS vs TABLE COVERAGE (added for the 2-yr run, 2026-09-05). The modulation
# table is on the ABSOLUTE mission clock and GALFOR_MODULATION_T0=data anchors
# its first epoch at the run's data_t0, so the data frame it covers is exactly
# its own span. Ask for more Tobs than that and the engine raises at build
# ("... covers [...] s but data span [...] s") -- job 433's crash, and the one
# failure mode a LONGER baseline newly exposes. The NOISE brick's /ltts delay
# table is generated over the same window, so this bounds both. Checked in
# seconds here instead of 20 minutes into an allocation.
_t = [float(ln.split()[0]) for ln in open(mod)
      if ln.strip() and not ln.lstrip().startswith("#")]
_span = max(_t) - min(_t)
print(f"[V8-NOISE] modulation span {_span:.6g} s ({_span / 86400.0:.3f} d, "
      f"{len(_t)} epochs) vs TOBS_TARGET {tobs:.6g} s ({tobs / 86400.0:.3f} d)")
if tobs > _span:
    print(f"[V8-NOISE] REFUSING: TOBS_TARGET {tobs:.6g} s exceeds the "
          f"modulation table's {_span:.6g} s of coverage by "
          f"{tobs - _span:.6g} s. Lower TOBS_TARGET (720 d = 62208000 s is "
          f"the tested value) or supply a longer GALFOR_MODULATION_PATH.")
    raise SystemExit(2)
if tobs > _span - 86400.0:
    print(f"[V8-NOISE] WARNING: only {_span - tobs:.6g} s of modulation "
          "coverage margin -- under a day. The anchor is data_t0, so any "
          "shift in the data start eats this.")
if not noise_file:
...
# GALFOR STREAM (joint run only). Resolved by FOLDER, not by find_file: the
# brick is GALFOR_731d_2.5s_L1.h5 -- right prefix, but no ``source0_`` infix,
# so the per-class resolver cannot see it. Also assert it is on the SAME grid
# as the NOISE brick, because the loader SUMS the two: a mismatched t0/dt/size
# would either assert deep in load_data or (worse) sum misaligned streams.
_gal_dir = os.path.join(mojito, "data", "GALFOR", "L1")
_gal = sorted(glob.glob(os.path.join(_gal_dir, "*.h5")))
if not _gal:
    print(f"[GALFOR] REFUSING: no .h5 under {_gal_dir!r}. This run needs the "
          "galactic-foreground stream; use submit_gf_noise_2yr.sh for the "
          "instrument-only control.")
    raise SystemExit(2)
if len(_gal) > 1 and not os.environ.get("MOJITO_GALFOR_FILE", "").strip():
    print(f"[GALFOR] REFUSING: {len(_gal)} .h5 files in {_gal_dir!r} "
          f"({[os.path.basename(g) for g in _gal]}); set MOJITO_GALFOR_FILE.")
    raise SystemExit(2)
_galf = os.environ.get("MOJITO_GALFOR_FILE", "").strip() or _gal[0]
if not os.path.isabs(_galf):
    _galf = os.path.join(_gal_dir, _galf)
with h5py.File(noise_file, "r") as fn, h5py.File(_galf, "r") as fg:
    _n, _g = dict(fn["tdis/sampling"].attrs), dict(fg["tdis/sampling"].attrs)
    _bad = {k: (_n[k], _g[k]) for k in ("t0", "dt", "size")
            if not (abs(float(_n[k]) - float(_g[k])) <= 1e-6 * max(1.0, abs(float(_n[k]))))}
    if _bad:
        print(f"[GALFOR] REFUSING: {os.path.basename(_galf)} is not on the "
              f"NOISE brick's grid (noise, galfor): {_bad}. The loader SUMS "
              "these streams; mismatched grids would corrupt the data.")
        raise SystemExit(2)
    _prov = str(fg.attrs.get("derived_by", ""))[:120]
print(f"[GALFOR] stream OK: {_galf}")
print(f"[GALFOR]   grid matches NOISE (t0={_g['t0']:.6f}, dt={_g['dt']}, "
      f"size={int(_g['size'])})")
print(f"[GALFOR]   provenance: {_prov}")
...
# ===================================================================
# NOISE-ONLY PSD VERIFICATION RUN @ 2 YEARS (2026-09-05, user request).
# Changes vs submit_gf_noise_v8.sh (the 90-d control): TOBS_TARGET
# 7776000 -> 62208000, the store/job naming, and the Tobs-vs-coverage
# preflight above. Nothing about the MODEL moves -- see the 2-YEAR
# header block at the top of this file.
#
# The note below is inherited from that 90-d script and still describes
# the variant and the coarse block correctly.
# -------------------------------------------------------------------
...
mpiexec -n 3 python scripts/run_global.py --stock noise_galfor_mojito
# python scripts/run_global.py --stock noise_galfor_mojito   # single-process fallback
