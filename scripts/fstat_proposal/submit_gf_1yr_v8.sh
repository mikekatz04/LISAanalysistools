...
export GB_N_SUBBANDS=2048  # PER GPU; per-slot cost ~4 MB @1yr (Tobs-linear) x 2 move caches -- HALVED TWICE from the 3-mo 8192 so the byte budget matches (8192@3mo->4096@6mo->2048@1yr). This is the #1 GPU-MEMORY WATCH: the old 1yr_v5 dropped further to 1024 under a v5-era memory ceiling; v8 retired the F-stat centers so 2048 should fit, but if GPU peaks approach the card ceiling, halve to 1024.   # PER GPU: total = x n_gpus
