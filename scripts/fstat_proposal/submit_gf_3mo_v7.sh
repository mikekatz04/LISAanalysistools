...
# ## "leaf-cap grid mismatch". A rewind + migration is required. The run    ##
# ## is cheap to rewind right now -- it was rewound at iteration 6 -- and   ##
# ## rewinding FIRST is what makes the migration exact rather than          ##
# ## approximate (see the note after the sequence).                         ##
# ##                                                                        ##
# ##   S=/shared/data/global_fit_output/gf_prod_3mo_v7/gf_prod_3mo_testing.h5
# ##                                                                        ##
...
# ##                                                                        ##
# ##   # 2. rewrite the cap grid in place. REFUSES if a .bak exists, so     ##
# ##   #    move any old one aside first.                                   ##
...
# ##                                                                        ##
# ##   # 4. resubmit                                                        ##
# ##   sbatch scripts/fstat_proposal/submit_gf_3mo_v7.sh                    ##
# ##                                                                        ##
# ## A FRESH STORE ALSO WORKS and needs no migration, but pays ~1-2 h to    ##
# ## refit the noise + VGB stages. The rewind keeps those.                  ##
# ##                                                                        ##
# ## WHY REWIND BEFORE MIGRATING. migrate_gb_cap_grid inherits per-cell     ##
# ## state from the OWNING band (cell c -> band c // K), which is exact for ##
# ## interior cells but only approximate for the one straddling cell per    ##
# ## seam -- and with live GB leaves, sources within half a cell of a seam  ##
# ## silently change cells. Rewound to zero leaves there is no occupancy to ##
# ## reattribute, so the migration is exactly "replace the edge values".    ##
# ## The ramp state comes along for free: band_leaf_cap / cap_cell_leaf_cap ##
...
# ## no ramped 1-8 cap is inherited into a half-width cell.                 ##
# ############################################################################
#
# ############################################################################
...
# regardless of this knob. Grep confirms it: `cap_drift_gate_edge_leak`
# has exactly two references in the tree -- its ctor read and that one
# clause -- so it can have no other effect here.
#
...
# cross-edge in-model moves from silently vanishing. The premise the
# short-circuit rests on ("in-model stays in its band window") is FALSE --
# the in-model window is the sub-band widened by N/4 bins per side, and
# gbbands pre-widens frequency_lims by another N/4 on RJ buffers ("allow
# to move over band edge when proposing in-model") -- so at divisor 1
# without this knob an in-model move can carry a leaf N/4 (N/2) bins into
# the neighbouring band with NOTHING checking that band's cap: the
# 2026-08-20 "29 leaves into a cap-1 cell" failure mode, at the seams.
...
#
# STILL CORRECT AT GB_CAP_DIVISOR=2 + STAGGER=1 (verified 2026-08-29), and
# the earlier "the clip folds an out-of-band proposal back into the leaf's
# own boundary cell" reading DOES NOT APPLY here. That clip lives in the
# NESTED branch only. The staggered branch clips solely to the GLOBAL cell
# range, so `b*K + floor((f - lo_b)/step_b + 1/2)` lets sub run past K-1
# (or below 0) and lands the leaf in the neighbouring straddling cell --
# which is exactly what the geometry wants. Verified against
# searchsorted(cap_edges, f) over +/- 0.9 band widths.
#
# On our UNIFORM band grid (GB_BAND_EDGES_MODE=uniform) the b*K terms
# cancel algebraically, so the staggered lookup gives the same cell for
# ANY handed-in band index and this knob is a no-op -- EXCEPT exactly on a
# cap edge, where the cancellation is exact in real arithmetic but not in
# floating point and the two band references can round the tie to
# different sides (measured: 2 of 37 sample points, both on edges;
# resolve_band matched searchsorted at both). So keep it ON: it costs one
# searchsorted, removes the tie ambiguity, and is genuinely load-bearing
# on a ragged get_n grid where the per-band steps differ.
...
#   simultaneous change with no evidence behind it; if flagship-style
#   stacking survives the staggered grid, a frequency-dependent ceiling is
#   still the next lever, not a global squeeze.
...
#   cell, is not foreign, and is NOT POLICED AT ALL -- correctly, since
#   the occupancy census does not change when a leaf moves within a cell.
#   So the headroom no longer governs the seam crossing that motivated it.
#   What it still governs are the two MID-BAND cap boundaries per band
