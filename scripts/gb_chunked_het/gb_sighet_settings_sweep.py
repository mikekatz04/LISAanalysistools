...
NR_CLR = {16: "#9ecae1", 32: "#4292c6", 64: "#08519c"}
...
def main():
    ...
    nrs = [int(x) for x in os.environ.get("SWEEP_NR", "16,32,64").split(",")]
    ...
    make_figure(out_dir, nts, nrs, Ks, bands, T_rep, res, nb)
        ...
        for nr in nrs:
            ...
            if xs:
                ax.plot(xs, ys, marker="o", lw=1.8,
                        color=NR_CLR.get(nr, "#08519c"), label=f"$n_r$={nr}")
        ax.axhline(allowed, color="#2f7d4f", lw=1.6, ls="--")
