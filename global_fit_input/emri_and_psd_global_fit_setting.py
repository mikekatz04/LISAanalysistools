"""Global-fit settings: EMRI + PSD run.
...
def setup_recipe(recipe, engine_info, curr, acs, priors, state):
    ...
    # Domain-aware EMRI waveform generator. Wraps the TD waveform and
    # projects onto the run's basis so ``ResidualAddOneRemoveOneMove``
    # receives a DomainBase (FDSignal / STFTSignal / WDMSignal).
    ...
    emri_pe_moves = GFCombineMove(
        moves=[emri_pe_move, psd_pe_move], share_temperature_control=False
    )
...
def get_general_erebor_settings() -> GeneralSetup:
    ...
    gpus = [3] if gpu_available else None
    if gpus is not None:
        cp.cuda.runtime.setDevice(gpus[0])
    nwalkers = 24
    ntemps = 10
