...
class GBSpecialStretchMove(StretchMove):
    ...
    def propose(self, model, state):
        ...
        N_vals = np.zeros_like(f_test, dtype=int)

        temp_f = f_test[f_test != 1e300]
        N_vals[f_test != 1e300] = get_N(np.full_like(temp_f, 1e-30), temp_f, self.waveform_kwargs["T"], self.waveform_kwargs["oversample"])
        ...
        for N_now in unique_N:
            ...
            f_test_copy[N_vals != N_now] = 1e300

            waveform_kwargs_now = self.waveform_kwargs.copy()
            waveform_kwargs_now["N"] = N_now

            f_test_sorted = np.asarray(np.sort(f_test_copy,axis=-1))
            inds_f_sorted = np.asarray(np.argsort(f_test_copy,axis=-1))  # np.tile(np.arange(f_test.shape[1]), (f_test.shape[0], 1)).reshape(ntemps, -1)
            groups = []
            group_len = []
