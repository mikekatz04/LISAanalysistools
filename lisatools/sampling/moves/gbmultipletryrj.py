...
class GBMutlipleTryRJ(MultipleTryMove, ReversibleJump, GBSpecialStretchMove):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.band_edges_fdot[:-1] = self.xp.asarray(get_fdot(self.band_edges[:-1], Mc=m_chirp_lims[0]))
        self.band_edges_fdot[1:] = self.xp.asarray(get_fdot(self.band_edges[1:], Mc=m_chirp_lims[1]))
    ...
    def special_like_func(self, generated_points, base_shape, inds_reverse=None, old_d_h_d_h=None, overall_inds=None):
        ...
        ll = self.gb.get_ll(prior_generated_points_in, self.mgh.data_list, self.mgh.psd_list, data_index=data_index, noise_index=noise_index, phase_marginalize=phase_marginalize, data_length=self.data_length,  data_splits=self.mgh.gpu_splits,  N=N_temp, return_cupy=True, **waveform_kwargs_in)
        ...
        if self.search:
            ...
            if self.xp.any(self.xp.isnan(prior_generated_points)) or self.xp.any(self.xp.isnan(phase_change)):
                breakpoint()
    ...
    def special_prior_func(self, generated_points, base_shape, inds_reverse=None, **kwargs):
        ...
        #st2 = time.perf_counter()
        lp_new = self.gpu_priors["gb_fixed"].logpdf(generated_points.reshape(-1, 8)).reshape(nwalkers, self.num_try)
        """et2 = time.perf_counter()
    ...
    def get_proposal(self, gb_coords, inds, changes, leaf_inds_for_changes, band_inds, random, supps=None, branch_supps=None):
        ...
        inds_reverse = inds_reverse_band[:2] + (leaf_inds_for_changes_gpu[inds_reverse_band],)
        ...
        inds_forward = inds_forward_band[:2] + (leaf_inds_for_changes_gpu[inds_forward_band],)
        ...
        if len(inds_here[0]) != 0:
            ...
            rj_info = dict(
                ll=self.xp.zeros_like(ll_here),
                lp=self.xp.zeros_like(lp_here)
            )
            ...
            if len(inds_reverse[0]) > 0:
                parameters_remove = self.parameter_transforms.both_transforms(gb_coords[inds_reverse_cpu], xp=self.xp)
    ...
    def propose(self, model, state):
        ...
        for rj_move_i in range(num_consecutive_rj_moves):
            ...
            temp_inds = self.xp.repeat(self.xp.arange(ntemps)[:, None], np.prod(coords_propose_in.shape[1:3]), axis=1).reshape(ntemps, nwalkers, -1)[inds_propose_in]
            walkers_inds = self.xp.repeat(self.xp.repeat(self.xp.arange(nwalkers)[None, :], ntemps, axis=0)[:, :, None],  coords_propose_in.shape[2], axis=2)[inds_propose_in]
            leaf_inds = self.xp.repeat(self.xp.arange(coords_propose_in.shape[2])[None, :], ntemps * nwalkers, axis=0).reshape(ntemps, nwalkers, -1)[inds_propose_in]

            # TODO: make this weighted somehow to not focus on empty bands
            # we are already leaving out the last one
            bands_per_walker = self.xp.tile((self.xp.arange(self.num_bands - 1)), (ntemps, nwalkers, 1))
            ...
            for i in range(2):
                ...
                unique_bands_inds_here, unique_bands_inds_here_index, unique_bands_inds_here_inverse, unique_bands_inds_here_counts = self.xp.unique(bands_inds_here_tmp, return_index=True, return_counts=True, return_inverse=True)
                ...
                band_index_count = self.xp.floor((unique_bands_inds_here - temp_index_count * int(1e12) - walkers_index_count * int(1e6))).astype(int)
                band_index_tmp = self.xp.searchsorted(bands_per_walker_here[0, 0], band_index_count, side="left")
                ...
                if self.xp.any(nleaves_here >= self.max_k[all_branch_names.index("gb_fixed")]):
                ...
                temp_inds_for_change = self.xp.repeat(self.xp.arange(ntemps)[:, None], nwalkers * num_sub_bands_here, axis=-1).reshape(ntemps, nwalkers, num_sub_bands_here)
                walker_inds_for_change = self.xp.tile(self.xp.arange(nwalkers), (ntemps, num_sub_bands_here, 1)).transpose((0, 2, 1))
                band_inds_for_change = self.xp.tile(self.xp.arange(num_sub_bands_here), (ntemps, nwalkers, 1))

                # leaves will be filled later
                leaf_inds_for_change = -self.xp.ones((ntemps, nwalkers, num_sub_bands_here), dtype=int)
                ...
                leaf_inds_tmp = self.xp.repeat(self.xp.arange(coords_propose_in.shape[2])[None, :], ntemps * nwalkers, axis=0).reshape(ntemps, nwalkers, -1)
                ...
                leaf_inds_tmp_2 = self.xp.sort(leaf_inds_tmp, axis=-1)[:, :, :num_sub_bands_here]
                ...
                tuple_accepted_reverse_cpu = tuple([tmp.get() for tmp in list(tuple_accepted_reverse)])
                # accepted removals
                new_state.branches["gb_fixed"].inds[tuple_accepted_reverse_cpu] = False
                delta_logl_trans1 = self.xp.zeros_like(leaf_inds_for_change, dtype=float)
                delta_logl_trans1[(
                    temp_inds_for_change.flatten()[accepted_inds_reverse], 
                    walker_inds_for_change.flatten()[accepted_inds_reverse],
                ...
                tuple_accepted_forward_cpu = tuple([tmp.get() for tmp in list(tuple_accepted_forward)])
                new_state.branches["gb_fixed"].inds[tuple_accepted_forward_cpu] = True
                new_state.branches["gb_fixed"].coords[tuple_accepted_forward_cpu] = new_coords[accepted_forward].get()
                ...
                if len(accepted_inds_forward) > 0:
                    ...
                    f0_accepted_addition = -100 * self.xp.ones_like(leaf_inds_for_change, dtype=float)
                    ...
                    for t in range(ntemps):
                        ...
                        f0_old = self.xp.asarray(state.branches["gb_fixed"].coords[t, state.branches["gb_fixed"].inds[t]][:, 1]) / 1e3
                        friend_start_inds = self.xp.asarray(state.branches["gb_fixed"].branch_supplimental.holder["friend_start_inds"][t, state.branches["gb_fixed"].inds[t]])
                        ...
                        inds_f0_accepted_addition = self.xp.searchsorted(f0_old_sorted, f0_accepted_addition_in2, side="right") - 1
                        ...
                        new_state.branches["gb_fixed"].branch_supplimental.holder["friend_start_inds"][(temp_inds_in.get(), walker_inds_in.get(), leaf_inds_in.get())] = new_friends_start_inds.get()
                ...
                delta_logl_trans2 = self.xp.zeros_like(leaf_inds_for_change, dtype=float)
                delta_logl_trans2[(
                    temp_inds_for_change.flatten()[accepted_inds_forward], 
                    walker_inds_for_change.flatten()[accepted_inds_forward],
                    band_inds_for_change.flatten()[accepted_inds_forward],
                ...
                if points_to_add_to_template.shape[0] > 0:
                    ...
                    inds_here_for_add = tuple([
                        self.xp.concatenate([tuple_accepted_forward[jjj], tuple_not_accepted_reverse[jjj]]) for jjj in range(len(tuple_not_accepted_reverse))
                    ])

                    inds_here_for_add_cpu = tuple([tmp.get() for tmp in list(inds_here_for_add)])

                    # inds_here_for_add = tuple_accepted_forward
                    
                    new_state.branches["gb_fixed"].branch_supplimental.holder["N_vals"][inds_here_for_add_cpu] = N_temp.get()
